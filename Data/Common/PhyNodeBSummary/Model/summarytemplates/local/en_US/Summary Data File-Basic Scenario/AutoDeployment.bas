...
Private Const SubNetwork_ValidLen = 255 'Name列的长度限制
...
Public Sub AutoDeploySheetChange(ByRef sheet As Worksheet, ByVal Target As range)
    ...
    maxColumnNumber = sheet.range("IV2").End(xlToLeft).column
    ...
    For Each cellRange In Target
        ...
        If cellRange.column = NameCol Then
            ...
            Call checkName(cellRange, String_ValidLen)
        ...
        ElseIf cellRange.column = SubNetworkCol Then
            ...
            Call checkName(cellRange, SubNetwork_ValidLen)
        Else
            '其余列做通用特殊字符的校验
            Call checkInvalidCharacter(cellRange, InvalidCharCommonStr, DisplayInvalidCharCommonStr)
...
'校验A列Name中的长度
Private Sub checkName(ByRef cellRange As range, ByRef maxLength As Integer)
    Dim nameValue As String
    Dim nResponse As Integer
    
    nameValue = Trim(cellRange.value)
    If cellRange.row > 2 And Len(nameValue) <> 0 Then
        If ((maxLength < LenB(StrConv(nameValue, vbFromUnicode)))) Or (1 > LenB(StrConv(nameValue, vbFromUnicode))) Then
            nResponse = MsgBox(getResByKey("Limited Length") & "[0~" & maxLength & "]", vbRetryCancel + vbCritical + vbApplicationModal + vbDefaultButton1, getResByKey("Warning"))
