...
Function get_EndRow() As Long
    ...
    Do While ShetDef_Shet.Cells(iRow, shtNameColNumInShtDef) <> ""
        If ShetDef_Shet.Cells(iRow, endRowColNumInShtDef).value <> "" Then
            If Int(ShetDef_Shet.Cells(iRow, endRowColNumInShtDef)) > get_EndRow Then
                get_EndRow = Int(ShetDef_Shet.Cells(iRow, endRowColNumInShtDef).value)
            End If
        End If
        iRow = iRow + 1
    Loop
