...
Private Sub DstIPGroup_Change()
    ...
    If IPRouteForm.DstIPSheet.value = getResByKey("A236") Then
...
Private Sub DstMaskGroup_Change()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").range("a65536").End(xlUp).row
        ...
        End If
    Next
    If IPRouteForm.DstMaskSheet.value = getResByKey("A237") Then
...
Private Sub AddButton_Click()
    ...
    For i = 0 To UBound(v)
        If v(i) = "" Then
            MsgBox getResByKey("A238"), vbExclamation, getResByKey("A239")
            flag = False
            Exit For
        End If
    Next
...
Private Sub DeleteButton_Click()
    If IPRouteForm.DstIPList.ListIndex = -1 Then
        MsgBox getResByKey("A240"), vbExclamation, getResByKey("A241")
    Else
...
Private Sub OKButton_Click()
    ...
    For i = 3 To Worksheets("IPRouteMap").range("a65536").End(xlUp).row
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A242") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 6).value = getResByKey("A243") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A244") Then
            For dstIpRowEnd = 1 To Worksheets(getResByKey("A245")).range("a65536").End(xlUp).row
                If Worksheets(getResByKey("A246")).Cells(dstIpRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
            dstIpRowEnd = dstIpRow + dstIpRowEnd - 1
        Else
            dstIpRowEnd = Worksheets(Worksheets("IPRouteMap").Cells(i, 1).value).range("a65536").End(xlUp).row
        End If
        
        
         If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A244") Then
            For sourceMaskRowEnd = 1 To Worksheets(getResByKey("A245")).range("a65536").End(xlUp).row
                If Worksheets(getResByKey("A246")).Cells(dstMaskRow + sourceMaskRowEnd, iPRouteMaskCol).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
            sourceMaskRowEnd = dstMaskRow + sourceMaskRowEnd - 1
        Else
            sourceMaskRowEnd = Worksheets(Worksheets("IPRouteMap").Cells(i, 6).value).range("a65536").End(xlUp).row
        End If
...
Function Get_ColOfCommon(sheetName As String, recordRow As Long, colName As String) As Long
    ...
    If f_flag = False Then
        ...
        For dstIpRowEnd = 1 To Worksheets(sheetName).range("a65536").End(xlUp).row
            If Worksheets(getResByKey("A247")).Cells(recordRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                Exit For
            End If
        Next
...
Function Get_IPRoute(dstIPValue As String, dstMaskValue As String) As String
    ...
    ElseIf regIp.test(dstIPValue) = False Then
        MsgBox dstIPValue & getResByKey("A248"), vbExclamation, getResByKey("A249")
    ElseIf regIp.test(dstMaskValue) = False Then
        MsgBox dstMaskValue & getResByKey("A250"), vbExclamation, getResByKey("A251")
    End If
