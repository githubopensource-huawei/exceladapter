...
Public Sub AddLnk()
    ...
    Call log.writeText("Begin AddLnk!")
    ...
    ' 激活Cover页到第一页
    Call log.writeText("Begin AddLnk active Cover begin")
    Dim coverSheet As Worksheet
    Set coverSheet = ThisWorkbook.Worksheets(getResByKey("Home"))
    coverSheet.Move Before:=ActiveWorkbook.Sheets(1)
    Call log.writeText("Begin AddLnk active Cover end")

    Call log.writeText("Begin AddLnk active move helpSheet begin")
    ...
    Call log.writeText("Begin AddLnk active move helpSheet end")

    coverSheet.Activate
    coverSheet.columns(1).ColumnWidth = 20
    coverSheet.columns(2).ColumnWidth = 40
    
    Call log.writeText("Begin AddLnk set coverSheet Width begin")
    If subFeatureSize > 0 Then
        ...
        Next i
    End If
    Call log.writeText("Begin AddLnk set coverSheet Width end")
    
    Call log.writeText("Begin AddLnk set Frame begin")
    If subFeatureSize > 0 Then
    ...
    End If
    Call log.writeText("Begin AddLnk set Frame end")

    'Hilight Tip Info Merge
    Call log.writeText("Begin AddLnk Hilight Tip Info Merge  begin")
    ...
    Call log.writeText("Begin AddLnk Hilight Tip Info Merge  end")
    
    Dim sheetCount As Long
    sheetCount = Sheets.count

    'Cover页处理--增加到各个Sheet的链接
    Call log.writeText("Begin AddLnk Cover Sheet Handle, add Link to every Sheet  begin")
    ...
    For sheetIndex = 1 To mocCnt
        ...
        With coverSheet.Cells(row, "B")
         ...
         row = row + 1
    Next sheetIndex
    Call log.writeText("Begin AddLnk Cover Sheet Handle, add Link to every Sheet  end")
    
    '大颗粒场景增加Template页签的链接，放在MO下面，合并成一个单元格
    Call log.writeText("Begin AddLnk add Link to template Sheet  begin")
    If subFeatureSize > 0 Then
        ...
        tempRange.Merge
    End If
    Call log.writeText("Begin AddLnk, add Link to template Sheet  end")
    
    'CoverSHeet Must Give
    Call log.writeText("Begin AddLnk, mustGive begin")
    ...
    End If
    coverSheet.Activate
    Call log.writeText("Begin AddLnk, mustGive end")
    ...
    Call log.writeText("Begin AddLnk, merge cells begin")
    'mergeStartRow = 8
    ...
    Call log.writeText("Begin AddLnk, merge cells end")
    
    '各个Sheet到Cover页的链接,单特性场景添加链接
    Call log.writeText("Begin AddLnk, single feature add link begin")
    ...
    Next sheetIndex
    Call log.writeText("Begin AddLnk, single feature add link end")
    
    Call log.writeText("Begin AddLnk, active help begin")
    If subFeatureSize > 0 Then
    ...
    helpSheet.Move Before:=ActiveWorkbook.Sheets(1)
    helpSheet.Activate
    Call log.writeText("Begin AddLnk, active help end")
    
    Call log.writeText("Begin AddLnk!")
    'If  there is no error exist, then delete  log.
    Dim logPath As String
    logPath = log.logPath
    log.closeFile
    Dim fs As New CFileSystemObject
    Call fs.deleteFile(logPath)
    
ErrorHandler:
    Call log.writeText("ErrorInfo = " & Err.Description)
End Sub
...
Private Sub mustGivesheet()
    ...
    Call log.writeText("Begin mustGivesheet!")
    Do
...
Private Sub refreshMappingDef(ByVal sheetName As String, ByVal attrName As String)
        ...
        Call log.writeText("End refreshMappingDef!")
End Sub


Private Sub SortWorksheets()
    Call log.writeText("Begin SortWorksheets!")
...
Private Sub NextSheet()
    Call log.writeText("Begin NextSheet!")
    
    If activeSheet.index <> Worksheets.count Then
        activeSheet.Next.Activate
    End If
    
    Call log.writeText("End NextSheet!")
End Sub


Private Sub setFrame(mergeStartRow As Integer, mergeEndRow As Integer)
    Call log.writeText("Begin setFrame!")
...
Private Sub setSubFeatureFrame(mergeStartRow As Integer, mergeEndRow As Integer, colCnt As Integer)
    ...
    Call log.writeText("End setSubFeatureFrame!")
End Sub


Private Sub setFrameFroMO(mergeStartRow As Integer, mergeEndRow As Integer)
    Call log.writeText("Begin setFrameFroMO!")
    ...
    Call log.writeText("End setFrameFroMO!")
End Sub


Private Sub setFrameNone(mergeStartRow As Integer, mergeEndRow As Integer)
    Call log.writeText("Begin setFrameNone!")
    ...
    Call log.writeText("End setFrameNone!")
End Sub
...
Private Sub setFrameNone4FeaturePkg(mergeStartRow As Integer, mergeEndRow As Integer, startCol As Integer, endCol As Integer)
    Call log.writeText("Begin setFrameNone4FeaturePkg!")
    ...
    Call log.writeText("End setFrameNone4FeaturePkg!")
End Sub


Private Sub readCfg()
    Call log.writeText("Begin readCfg!")
    ...
    Call log.writeText("End readCfg!")
End Sub
...
Function readUTF8File(strFile As String) As String
    ...
    Call log.writeText("readUTF8File Begin")
    ...
    Call log.writeText("readUTF8File End")
    Exit Function
errHandle:
...
Private Sub refreshComment(ByRef myRange As Range)
    ...
    Call log.writeText("refreshComment Begin!")
...
On Error Resume Next
    ...
    Call log.writeText("setComment End!")
End Sub
...
On Error Resume Next
    Application.DisplayAlerts = False
    Call log.writeText("mergeSpecialColumns Begin!")
    ...
    Call log.writeText("mergeSpecialColumns End!")
End Sub
