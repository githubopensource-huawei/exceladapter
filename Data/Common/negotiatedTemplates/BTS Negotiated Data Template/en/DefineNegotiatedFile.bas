...
End If
FieldCol = Trim(SheetDefine(iRow, FieldDefCol))
FieldPostil = FieldPostil + Chr(13) + Chr(10) + SheetDefine(iRow, DetailDisplayCol)
CurSheet.Range(FieldCol + FieldRow).ClearComments
CurSheet.Range(FieldCol + FieldRow).AddComment FieldPostil
CurSheet.Range(FieldCol + FieldRow).Comment.Shape.Width = 200
CurSheet.Range(FieldCol + FieldRow).Comment.Shape.Height = 200
...
'通过FieldMapDef的XML名称生成home页面的对象，
'有些对象与模型名称不一致，需要转换，这些对象都被命名为MOName+"_PLAN_IMPORT",
'所以在生成home页面模型名称时，统一去除_PLAN_IMPORT
Public Sub RefreshHome()
  
  Const POSTFIX = "_PLAN_IMPORT"
  Const COLDEF = 2
  Const MOCDEF = 3
  Dim iFieldMapRowIndex As Integer
  Dim iHomeRowIndex As Integer
  Dim iLanguage As Integer
  Dim strLinkAddr As String
  Dim strLinkSheet As String
  Dim strColDef As String
  Dim strLastMoc As String
  Dim strCurMoc As String
  Dim shtHomeSheet As Worksheet
  Dim shtFieldMapSheet As Worksheet
  
  Set shtHomeSheet = Sheets("Home")
  Set shtFieldMapSheet = Sheets("FieldMapDef")
  iFieldMapRowIndex = 2 'FieldMap定义从第二行开始
  iHomeRowIndex = 2 'Home页面的MO对象列表从第二行开始
  strColDef = Trim(shtFieldMapSheet.Cells(iFieldMapRowIndex, COLDEF))
  strLastMoc = ""
  strCurMoc = Trim(shtFieldMapSheet.Cells(iFieldMapRowIndex, MOCDEF))
  Sheets("Home").Rows("2:65536").ClearContents
  
  While strColDef <> "" And strCurMoc <> ""
    strCurMoc = Replace(strCurMoc, POSTFIX, "")
    If strCurMoc <> strLastMoc And strCurMoc <> "GCELLFREQ_FREQ" And _
      shtHomeSheet.Columns(1).Find(strCurMoc, LookIn:=xlValues, lookat:=xlWhole, MatchCase:=True) Is Nothing Then
       strLinkAddr = ConvertToLetter(CInt(strColDef)) & "6"
       strLinkSheet = shtFieldMapSheet.Cells(iFieldMapRowIndex, 1)
       With shtHomeSheet
         .Hyperlinks.Add .Cells(iHomeRowIndex, 1), "", _
         SubAddress:="'" & strLinkSheet & "'!" & strLinkAddr, _
         TextToDisplay:=strCurMoc
       End With
       iHomeRowIndex = iHomeRowIndex + 1
    End If
    
    iFieldMapRowIndex = iFieldMapRowIndex + 1
    If (shtFieldMapSheet.Cells(iFieldMapRowIndex - 1, 1) = shtFieldMapSheet.Cells(iFieldMapRowIndex, 1) _
      And shtFieldMapSheet.Cells(iFieldMapRowIndex, 4) <> "BTSNAME" _
      And shtFieldMapSheet.Cells(iFieldMapRowIndex, 4) <> "CELLNAME") _
      Or shtFieldMapSheet.Cells(iFieldMapRowIndex - 1, 1) <> shtFieldMapSheet.Cells(iFieldMapRowIndex, 1) Then
      strLastMoc = strCurMoc
      strColDef = Trim(shtFieldMapSheet.Cells(iFieldMapRowIndex, COLDEF))
      strCurMoc = Trim(shtFieldMapSheet.Cells(iFieldMapRowIndex, MOCDEF))
    End If
  Wend
  ...
  With shtHomeSheet
    .Hyperlinks.Add .Cells(iHomeRowIndex, 1), "", _
    SubAddress:="'" & "GTRX" & "'!L6", _
    TextToDisplay:="TRXBIND2PHYBRD"
  End With
