Attribute VB_Name = "IUCSSheet"
Option Explicit

Const TableBeginCol = 2
Const TableEndCol = 42 + 1
...
Public Sub IUCS_Ensure_NoValue(ByVal Target As Range)
  ...
  For Each CurRange In Target
    If CurRange.Cells(1, 1) <> "" And CurRange.Cells(1, 1).Interior.ColorIndex = SolidColorIdx And CurRange.Cells(1, 1).Interior.Pattern = SolidPattern Then
      ...
      CurRange.Cells(1, 1).ClearContentsss
    End If
  Next CurRange
End Sub
...
Public Sub IUCS_RefreshThisSheet(ByVal Target As Range, isActivateFlag As Boolean)
    ...
    Call IUCS_Ensure_NoValue(Target)
