...
Sub Execute_Branch_Control(ByVal sheet As Worksheet, ByVal cellRange As range, contRel As controlRelation, ByRef currentNeType As String)
    ...
    For m = 0 To contRel.contedNum
        For conRowNum = 2 To controldef.range("a65536").End(xlUp).row
            ...
            If contRel.mocName = mocName And contRel.neType = neType And contRel.contedAttrs(m) = attrName And contRel.sheetName = sheetName And contRel.groupName = groupName Then
...
Function get_colNum(ByVal sheetName As String, ByVal groupName As String, ByVal columnName As String, rowNum As Long) As Long
    ...
    End If
End Function
Function get_MocAndAttrcolNum(ByVal mocName As String, ByVal attrName As String, ByVal sheetName As String, groupName As String) As Long
    
    Dim controldef As Worksheet
    Set controldef = ThisWorkbook.Worksheets("CONTROL DEF")
    get_MocAndAttrcolNum = 0
    
    Dim rowIdx, noUse As Long
    Dim columnName As String
    With controldef
        For rowIdx = 2 To .range("a65536").End(xlUp).row
            If mocName = .Cells(rowIdx, 1).value And attrName = .Cells(rowIdx, 2).value _
                And sheetName = .Cells(rowIdx, 7).value And groupName = .Cells(rowIdx, 8).value Then
                columnName = .Cells(rowIdx, 9).value
