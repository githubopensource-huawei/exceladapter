...
Private Const comment_shtNameCol As Integer = 1
Private Const comment_grpNameCol As Integer = 2
Private Const comment_colNameCol As Integer = 3
Private Const comment_commentCol As Integer = 4
Private Const comment_markupCol As Integer = 5

Public Const APPEND As Integer = 0
Public Const INSERT As Integer = 1
Public Const OVERWRITE As Integer = 2


'==============================interface===============================
Public Function existsComment(shtName As String, grpName As String, colName As String, commentText As String) As Boolean
On Error GoTo ErrorHandler
    ...
    Set targetRange = commentSht.columns(comment_colNameCol).Find(colName, LookIn:=xlValues, lookat:=xlWhole)
    If Not targetRange Is Nothing Then
        firstAddr = targetRange.address
        Do
            If targetRange.Offset(0, comment_grpNameCol - comment_colNameCol).value = grpName _
                And targetRange.Offset(0, comment_shtNameCol - comment_colNameCol).value = shtName Then Exit Do
                
            Set targetRange = commentSht.columns(comment_colNameCol).FindNext(targetRange)
        Loop While Not targetRange Is Nothing And targetRange.address <> firstAddr
    End If
    
    If targetRange Is Nothing Then
        Debug.Print "[CommentsAssist::existsComment]cannot find comment info by(" & shtName & "," & grpName & "," & colName & ")"
        Exit Function
    End If
    
    If targetRange.Offset(0, comment_commentCol - comment_colNameCol).value = commentText Then existsComment = True
    Exit Function
ErrorHandler:
    Debug.Print "some exception in existsComment, " & Err.Description
End Function

Public Function getComment(shtName As String, grpName As String, colName As String) As String
On Error GoTo ErrorHandler
    getComment = ""
    Dim commentSht As Worksheet
    Set commentSht = ThisWorkbook.Worksheets("Comments")
    ...
    If targetRange Is Nothing Then
        Debug.Print "[CommentsAssist::getComment]cannot find comment info by(" & shtName & "," & grpName & "," & colName & ")"
        Exit Function
    End If
    
    getComment = targetRange.Offset(0, comment_commentCol - comment_colNameCol).value
    Exit Function
ErrorHandler:
    Debug.Print "some exception in getComment, " & Err.Description
End Function

'way:insert(insert at then beginning),append(append to the end),overwrite
Public Function modifyComment(shtName As String, grpName As String, colName As String, newComment As String, Optional ByVal way As Integer = APPEND) As String
On Error GoTo ErrorHandler
    modifyComment = ""
    Dim commentSht As Worksheet
    Set commentSht = ThisWorkbook.Worksheets("Comments")
    ...
    If targetRange Is Nothing Then
        Debug.Print "[CommentsAssist::modifyComment]cannot find comment info by(" & shtName & "," & grpName & "," & colName & ")"
        Exit Function
    End If
    
    Dim oldComment As String
    oldComment = targetRange.Offset(0, comment_commentCol - comment_colNameCol).value
    If way = APPEND Then
        If oldComment = "" Then
            modifyComment = newComment
        Else
            modifyComment = oldComment & vbCr & vbLf & vbCr & vbLf & newComment
        End If
    ElseIf way = INSERT Then
        If newComment = "" Then
            modifyComment = oldComment
        Else
            modifyComment = newComment & vbCr & vbLf & vbCr & vbLf & oldComment
        End If
    ElseIf way = OVERWRITE Then
        modifyComment = newComment
    End If
    
    targetRange.Offset(0, comment_commentCol - comment_colNameCol).value = modifyComment
    targetRange.Offset(0, comment_markupCol - comment_colNameCol).value = "Y"
    Exit Function
ErrorHandler:
    Debug.Print "some exception in modifyComment, " & Err.Description
End Function
    ...
    With sht
        For colIdx = 1 To .range("IV" & listShtAttrRow).End(xlToLeft).column
            ...
            If commentsMgr.hasKey(key) Then
                ...
                If Not oldComment Is Nothing Then target.comment.Delete
