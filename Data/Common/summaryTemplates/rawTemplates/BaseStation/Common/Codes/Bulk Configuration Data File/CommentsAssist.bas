...
Private shetTypes As CMap


'==============================interface===============================
Public Sub loadComments(beginPercent As Integer)
On Error GoTo ErrorHandler
    Debug.Print "loadComments entered..."
    Dim t As Date
    t = Timer
    Set shetTypes = New CMap
    ...
    Dim rowIdx As Integer, rowCount As Integer
    With commentSht
        ...
        For rowIdx = 2 To rowCount
            ...
            If beginPercent > 0 Then
                Dim percent As Integer
                percent = rowIdx / rowCount * beginPercent
...
Public Sub addAllComments()
    ...
    Dim beginPercent As Integer
    beginPercent = 20
    If commentInfos Is Nothing Then loadComments (beginPercent)
    ...
    With ThisWorkbook
        For idx = 1 To targetShts.count
            ...
            percent = beginPercent + idx / targetShts.count * (100 - beginPercent)
            Call ProgressBar.updateProgress(percent)
        Next
    End With
...
Public Sub addCommentsBySheet(sht As Worksheet)
    ...
    If commentInfos Is Nothing Then loadComments (0)
...
Private Function getShtType(shtName As String) As String
    ...
    If shetTypes.hasKey(shtName) = True Then
        getShtType = shetTypes.GetAt(shtName)
    End If
        
    If containsASheet(ThisWorkbook, shtName) Then
        If ThisWorkbook.Worksheets(shtName).Tab.colorIndex = BluePrintSheetColor Then
            If shetTypes.hasKey(shtName) = False Then
                Call shetTypes.SetAt(shtName, shtType_IUB)
            End If
    ...
    If InStr(shtName, getResByKey("Board Style")) = 1 Then
        ...
        If shetTypes.hasKey(shtName) = False Then
                Call shetTypes.SetAt(shtName, shtType_Board)
        End If
        Exit Function
    End If
    ...
    If shtType = shtType_Main Or shtType = shtType_List Or shtType = shtType_Pattern Then
        If shetTypes.hasKey(shtName) = False Then
                Call shetTypes.SetAt(shtName, shtType)
        End If
        Exit Function
    End If
