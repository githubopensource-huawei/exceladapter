...
Public Function getIndirectValidateListValue(ByRef mocSheetName As String, ByRef mocGroupName As String, ByRef mocColumnName As String) As String
    On Error GoTo ErrorHandler
    Dim mappingDef As CMappingDef
    ...
    For index = 2 To validSheet.range("a65536").End(xlUp).row
     If GetDesStr(validSheet.Cells(index, 1).value) = GetDesStr(currmocName) And GetDesStr(validSheet.Cells(index, 2).value) = GetDesStr(currattrname) Then
        ...
        getIndirectValidateListValue = "=INDIRECT(""'VALID DEF'!C" & CStr(index) & ":" & getColStr(maxColNum) & CStr(index) & """)"
     End If
    Next
    Exit Function
ErrorHandler:
...
'                         MsgBox getResByKey("AddListReferenceWarning"), vbExclamation + vbOKCancel, getResByKey("CommonData")
'                         Exit Sub
'                    End If
...
'                    '     MsgBox Replace(getResByKey("SelectFirstSheet"), "%1", getFisrtNodeBSheet), vbExclamation + vbOKCancel, getResByKey("CommonData")
'                    '    Exit Sub
'                    'End If
...
Private Sub getRowNumAndColumnNum(sheetName As String, groupName As String, columnName As String, rowNum As Long, columnNum As Long)
    ...
    If sheetName = getResByKey("CommonData") Then
...
Private Function checkListValidity(sheetName As String) As Boolean
    ...
    For Each cell In Selection
        If ThisWorkbook.Worksheets(sheetName).Cells(cell.row, 1).value <> ThisWorkbook.Worksheets(sheetName).Cells(3, 1).value Then
            MsgBox getResByKey("SelectFirstNE"), vbExclamation + vbOKCancel, getResByKey("CommonData")
    ...
    For Each cell In Selection
        If isInit = True And cell.value <> value1 Then
            ...
            & Chr(13) & getResByKey("SureToContinue"), vbYesNo + vbQuestion, getResByKey("CommonData"))
