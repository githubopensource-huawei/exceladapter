...
Sub GenOneSiteData(Summary_Shet As Worksheet, CurSiteName As String, ReferenceSiteName As String, CurRow As Long)
  ...
  End If
ErrorHandler:
Debug.Print "GenOneSiteData error" & Err.Description & iColumn
End Sub
Sub Process_DefSite()
  Dim flag As Long
  Dim ShtIdx As Long
...
Private Function checkSameSitesName() As Boolean
    ...
    Dim siteNameCol As Long
    siteNameCol = siteNameColNum(baseStationSheet)
    
    NodeBEndRow = baseStationSheet.range(getColStr(siteNameCol) & "65535").End(xlUp).row
    ...
    For rowNumber = NodeBBeginRow To NodeBEndRow
        siteName = Trim(baseStationSheet.range(getColStr(siteNameCol) & rowNumber).value)
