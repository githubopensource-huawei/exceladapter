...
Private Sub setFrameCaptions()
    Me.Caption = getResByKey("Bar_IPRoute")
    Me.DstIpFrame.Caption = getResByKey("DstIPSettingCaption")
    Me.MaskFrame.Caption = getResByKey("MaskSettingCaption")
    Me.SheetLabel.Caption = getResByKey("SheetCaption")
    Me.GroupLabel.Caption = getResByKey("GroupCaption")
    Me.ColumnLabel.Caption = getResByKey("ColumnCaption")
    Me.ResultGroupLabel.Caption = getResByKey("FillingGroupCaption")
    Me.ResultColumnLabel.Caption = getResByKey("FillingColCaption")
    Me.SheetLabel2.Caption = getResByKey("SheetCaption")
    Me.GroupLabel2.Caption = getResByKey("GroupCaption")
    Me.ColumnLabel2.Caption = getResByKey("ColumnCaption")
    Me.ResultGroupLabel2.Caption = getResByKey("FillingGroupCaption")
    Me.ResultColumnLabel2.Caption = getResByKey("FillingColCaption")
    Me.IPSelect.Caption = getResByKey("SelectFillingColCaption")
    Me.IPAdd.Caption = getResByKey("AddFillingColCaption")
    Me.SetDataFrame.Caption = getResByKey("SettingDataListCaption")
    Me.ConvertFrame.Caption = getResByKey("SettingConvertModeCaption")
    Me.IpAddrButton.Caption = getResByKey("NetIpAddrCaption")
    
    Me.MaskSelect.Caption = getResByKey("SelectFillingColCaption")
    Me.MaskAdd.Caption = getResByKey("AddFillingColCaption")
    
    Me.AddButton.Caption = getResByKey("AddButtonCaption")
    Me.DeleteButton.Caption = getResByKey("DeleteButtonCaption")
    Me.OKButton.Caption = getResByKey("OKButtonCaption")
    Me.CancelButton.Caption = getResByKey("CancelButtonCaption")
End Sub

'从「SHEET DEF」页获取Sheet页侯选值
Private Sub SetDstSheet()
    For m_rowNum = 2 To Worksheets("SHEET DEF").range("a65536").End(xlUp).row
...
Private Sub DstIPGroup_Change()
    ...
    If IPRouteForm.DstIPSheet.value = getResByKey("Common Data") Then
...
Private Sub DstMaskGroup_Change()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").range("a65536").End(xlUp).row
        ...
        End If
    Next
    If IPRouteForm.DstMaskSheet.value = getResByKey("Common Data") Then
...
Private Sub AddButton_Click()
    ...
    For i = 0 To UBound(v)
        If v(i) = "" Then
            MsgBox getResByKey("PLEASE_SET_DATA"), vbExclamation, getResByKey("Warning")
            flag = False
            Exit For
        End If
    Next
    ...
    If flag = True Then
        rowCount = Worksheets("IPRouteMap").range("a65536").End(xlUp).row
...
Private Sub OKButton_Click()
    ...
    For i = 3 To Worksheets("IPRouteMap").range("a65536").End(xlUp).row
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("Common Data") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 6).value = getResByKey("Common Data") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("Common Data") Then
            For dstIpRowEnd = 1 To Worksheets(getResByKey("Common Data")).range("a65536").End(xlUp).row
                If Worksheets(getResByKey("Common Data")).Cells(dstIpRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
            dstIpRowEnd = dstIpRow + dstIpRowEnd - 1
        Else
            dstIpRowEnd = getUsedRowCount(Worksheets(Worksheets("IPRouteMap").Cells(i, 1).value))
        End If
        
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("Common Data") Then
            For sourceMaskRowEnd = 1 To Worksheets(getResByKey("Common Data")).range("a65536").End(xlUp).row
                If Worksheets(getResByKey("Common Data")).Cells(dstMaskRow + sourceMaskRowEnd, iPRouteMaskCol).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
            sourceMaskRowEnd = dstMaskRow + sourceMaskRowEnd - 1
        Else
            sourceMaskRowEnd = getUsedRowCount(Worksheets(Worksheets("IPRouteMap").Cells(i, 6).value))
        End If
        ...
        For dstIpRow = dstIpRow + 1 To dstIpRowEnd
            ...
            If dstIPValue <> "" And dstMaskValue <> "" Then
                If IPRouteForm.IpAddrButton.value = True Then
...
Function Get_ColOfCommon(sheetName As String, recordRow As Long, colName As String) As Long
    ...
    m_ColEnd = Worksheets(sheetName).range("IV" + CStr(recordRow)).End(xlToLeft).column
    ...
    If f_flag = False Then
        ...
        For dstIpRowEnd = 1 To Worksheets(sheetName).range("a65536").End(xlUp).row
            If Worksheets(getResByKey("Common Data")).Cells(recordRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                Exit For
            End If
        Next
        ...
        Dim mappingRow
        mappingRow = Worksheets("MAPPING DEF").range("a65536").End(xlUp).row + 1
...
Function Get_ColOfNormal(sheetName As String, groupName As String, colName As String) As Long
    ...
    m_ColStart = Worksheets(sheetName).range("IV2").End(xlToLeft).column
    m_ColEnd = m_ColStart
    For m_colNum = 1 To Worksheets(sheetName).range("IV1").End(xlToLeft).column
        If groupName = Worksheets(sheetName).Cells(1, m_colNum).value Then
            ...
            For m_ColEnd = m_colNum + 1 To Worksheets(sheetName).range("IV2").End(xlToLeft).column
    ...
    If f_flag = False Then
        ...
        Dim rowCount
        For rowCount = 2 To Worksheets(sheetName).range("a65536").End(xlUp).row
...
Function Get_IPRoute(dstIPValue As String, dstMaskValue As String) As String
    ...
    If regIp.test(dstIPValue) = False Then MsgBox dstIPValue & getResByKey("inValidIP"), vbExclamation, getResByKey("Warning")
    
    If regIp.test(dstMaskValue) = False Then
        MsgBox dstMaskValue & getResByKey("inValidMask"), vbExclamation, getResByKey("Warning")
        dstMaskValue = ""
    End If
...
Function Check_Sheet(sheetValue As String) As Boolean
    ...
    Set cell = Worksheets("MAPPING DEF").range("A:A").Find(what:=sheetValue, lookat:=xlWhole)
    firstAddress = cell.address(0, 0)
    If Not cell Is Nothing Then
        Do
            targetRow = cell.row
            group_name = sheetValue + "." + Worksheets("MAPPING DEF").Cells(targetRow, 2).value
            flag = Check_Col(group_name + "." + Worksheets("MAPPING DEF").Cells(targetRow, 3).value)
            If flag Then Exit Do
            Set cell = Worksheets("MAPPING DEF").range("A:A").FindNext(cell)
