...
Private valideDefCollection As Collection

Sub initInnerValideDef()
        If valideCollection Is Nothing Then
            Set valideCollection = New Collection
        End If
        If valideDefCollection Is Nothing Then
            Set valideDefCollection = New Collection
        End If
        ...
        Next
        'VALID DEF
        Set innerSheet = ThisWorkbook.Worksheets("VALID DEF")
        rowCount = innerSheet.range("a65536").End(xlUp).row
        
        For index = 2 To rowCount
                Set valideDef = New CValideDef
                valideDef.mocName = innerSheet.Cells(index, 1).value
                valideDef.attrName = innerSheet.Cells(index, 2).value
                valideDef.row = index
                valideDef.column = innerSheet.range("IV" + CStr(index)).End(xlToLeft).column
                Call valideDefCollection.Add(valideDef, valideDef.getMocKey)
        Next
End Sub

Function isValidDef(key As String) As Boolean
If valideDefCollection Is Nothing Then
    Call initInnerValideDef
End If
Dim isExist As Boolean
isExist = False
isExist = Contains(valideDefCollection, key)
isValidDef = isExist
End Function
...
Function getInnerValideDef(key As String) As CValideDef
        ...
        Else
            isExist = Contains(valideDefCollection, key)
            If isExist Then
                Set getInnerValideDef = valideDefCollection.Item(key)
            End If
        End If
End Function
