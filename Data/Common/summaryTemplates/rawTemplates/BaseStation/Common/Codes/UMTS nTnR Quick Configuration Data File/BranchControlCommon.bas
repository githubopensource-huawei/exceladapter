...
Sub buildControlRelations()
    ...
    With controlDef
        For conRowNum = 2 To .range("a65536").End(xlUp).row
...
Public Sub currentParameterBranchCheck(ByRef ws As Worksheet, ByRef cellRange As range)
    ...
    If isBoardStyleSheet(ws) Then
        ...
        grpName = getGroupName(ws, cellRange.row)
        If grpName <> getResByKey("BBP") And grpName <> getResByKey("BBEqm") Then Exit Sub
    End If
...
Public Function getRealControlInfo(conInfor As String) As String
    ...
    With controlInforSht
        ...
        Dim cell As range
        For Each cell In .range("A" & targetRow & ":" & C(maxCol) & targetRow)
