...
Private Function getColMaxLength(sheet As Worksheet) As Long
    getColMaxLength = sheet.range("IV1").End(xlToLeft).column
End Function
...
'            End If
            If CELL_TYPE = 1 And row(8) <> "" Then
                tempSheet.Cells(index, SectorEqm_Index).value = row(8)
            End If
            index = index + 1
        Next
NextLoop:
...
Private Function insertCellIdColumn() As Boolean
        ...
        For index = 2 To cellsheet.range("a65536").End(xlUp).row
            If cellsheet.Cells(index, 1).value = SITE_NAME And cellsheet.Cells(index, 2).value <> "RMV" Then
                If cellsStr <> "" Then
...
'                ThisWorkbook.ActiveSheet.Cells(1, Trx_Index).Comment.text text:=getResByKey("TRXIDINFO")
'
'        Dim cellRang As Range
'        Set cellRang = ThisWorkbook.ActiveSheet.Range("B2:B" + CStr(2 + ROW_COUNT + 3))
'        If alltrxlistStr <> "" Then
'                trxValueStr = alltrxlistStr
'                With cellRang.Validation
'                   .Delete
'                   .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:=xlBetween, Formula1:=alltrxlistStr
'                End With
'                ThisWorkbook.ActiveSheet.Columns(Trx_Index).EntireColumn.AutoFit
'        End If
'End Sub
...
Private Function insertSectorIdColumn() As Boolean
        ...
        Dim sectorSht As Worksheet
        Set sectorSht = ThisWorkbook.Worksheets(getResByKey("USectorSheet"))
        
        Dim nodeName As String
        nodeName = getNodeName(SITE_NAME)
        
        Dim sectorsStr As String
        sectorsStr = ""

        Dim index As Long
        For index = 3 To sectorSht.range("a65536").End(xlUp).row
            If sectorSht.Cells(index, 1) = nodeName Then
                sectorsStr = sectorsStr & sectorSht.Cells(index, 2) & ","
            End If
        Next
        sectorsStr = Left(sectorsStr, Len(sectorsStr) - 1)

        Dim cellRang As range
        Set cellRang = ThisWorkbook.ActiveSheet.range("B2:B" + CStr(2 + ROW_COUNT + 3))
        With cellRang.Validation
           .Delete
           .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:=xlBetween, Formula1:=sectorsStr
        End With
...
'        End If
        ...
        For Each brdGrp In grpCollection
            ...
            With ThisWorkbook.ActiveSheet.Cells(1, Board_Index)
                .comment.text text:=getResByKey("RxuBoard")
                .Interior.colorIndex = COLUMN_NAME_COLOR
            End With
            
            For index = startRow + 2 To endRow
                If boardStyleSheet.Cells(index, 1) <> "RMV" Then
                    If brdStr = "" Then
...
Public Function getboradNoColumNumber(ByRef ws As Worksheet, ByRef rowNumber As Long, ByRef groupName As String) As Long
    ...
    For colIndex = 1 To ws.range("IV2").End(xlToLeft).column
...
'    End If
    ...
    For index = 3 To cellsheet.range("a65536").End(xlUp).row
        If cellsheet.Cells(index, 1).value = SITE_NAME And cellsheet.Cells(index, constCellTempCol).value <> "" And cellsheet.Cells(index, 2).value <> "RMV" Then
...
'    If sheetName = "GSM Cell" Or sheetName = getResByKey("A184") Then
'        cellSheetType = 0
'    ElseIf sheetName = "UMTS Cell" Or sheetName = getResByKey("A185") Then
        cellSheetType = 1
'    ElseIf sheetName = "LTE Cell" Or sheetName = getResByKey("A186") Then
'        cellSheetType = 2
'    Else
'        cellSheetType = -1
'    End If
...
'    End If
    ...
    ThisWorkbook.ActiveSheet.Cells(1, Ante_Index).comment.text text:=getResByKey("Antenne")
...
'        End If
        ...
        With ThisWorkbook.ActiveSheet.Cells(1, Model_Index)
            .value = getResByKey("anteModel")
            .comment.text text:=getResByKey("AnteModelInfo")
            .Interior.colorIndex = COLUMN_NAME_COLOR
        End With
...
Private Function insertSectoreqmColumn() As Boolean
        ...
        cellsheet.Cells(2, columnIndex).Copy Destination:=ThisWorkbook.ActiveSheet.Cells(1, SectorEqm_Index)
                ThisWorkbook.ActiveSheet.Cells(1, SectorEqm_Index).comment.text text:=getResByKey("UMTSSECTOREQMPROPERTY")
...
Private Sub writeUMTSOneCell(tmpsheet As Worksheet, cellsheet As Worksheet, cellId As String, _
...
'        End If
        ...
        End If
        errflag = checkInputData(tmpsheet, rowNum, cellStr, sectorStr, boardStr, antnStr, modelStr, baseEqmStr, trxStr, sectoreqmproStr, errRangeCol)
        If errflag Then
            If CELL_TYPE = 0 Then
'                trxStr = tmpsheet.Cells(rowNum, Trx_Index).value
...
'                Call sectorBaseEqmMap.SetAt(sectorBaseEqm, bMap)
            ElseIf CELL_TYPE = 1 And sectoreqmproStr <> "" Then
                keyStr = cellStr + "_" + sectorStr + "_" + boardStr + "_" + antnStr + "_" + modelStr + "_" + sectoreqmproStr
                
                sectoreqmPro = cellStr + "_" + sectorStr
                Dim sbMap As CMap
                Set sbMap = New CMap
        ...
        ElseIf columnRange.column = sectoIdCol Then
            For Each cellRange In columnRange
                ...
                If target.row > 1 And sectoIdValue <> "" Then
                    If CStr(sectoIdValue) < 0 Or CStr(sectoIdValue) > 65535 Then
                        nResponse = MsgBox(getResByKey("Range") & "[0~65535]", vbRetryCancel + vbCritical + vbApplicationModal + vbDefaultButton1, getResByKey("Warning"))
                        If nResponse = vbRetry Then
                            cellRange.Select
                        End If
                        cellRange.value = ""
...
'        cellId = sheet.Cells(rowNumber, 1).value
'
'        For Each sVal In GCell_TrxListInfo.KeyCollection
'            trxlistStr = GCell_TrxListInfo.GetAt(sVal)
'            If CStr(sVal) = cellId And trxlistStr <> "" Then
...
Public Function isUmtsSectoreqmPo() As Boolean
    ...
    actSheetName = getResByKey("UMTSCellSheet")
