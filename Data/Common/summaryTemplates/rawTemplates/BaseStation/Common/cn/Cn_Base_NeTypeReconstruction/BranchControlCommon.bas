                ...
                While InStr(1, conInfor, "attribute", vbBinaryCompare) <> 0
                    ...
                    If ControlRelationNum > 0 Then
                        For i = 0 To ControlRelationNum - 1
                            If mocName = ControlRelMap(i).mocName And contName = ControlRelMap(i).contAttr And neType = ControlRelMap(i).neType And _
                                sheetName = ControlRelMap(i).sheetName And groupName = ControlRelMap(i).groupName Then
                                flag = False
                                ...
                                If flag = False Then
                                    ...
                                    For index1 = 0 To ControlRelationNum - 1
                                        If mocName = ControlRelMap(index1).mocName And contedName = ControlRelMap(index1).contAttr And _
                                            neType = ControlRelMap(index1).neType And sheetName = ControlRelMap(index1).sheetName And groupName = ControlRelMap(index1).groupName Then
                    ...
                    If isExist = False Then
                        ...
                        ControlRelMap(ControlRelationNum).groupName = groupName
...
Public Sub buildMappingNumberRelations()
    ...
    Dim mocName As String, attrName As String, neType As String, sheetName As String, groupName As String
    For rowIndex = 2 To mappingDefSheet.range("a65535").End(xlUp).row
        sheetName = mappingDefSheet.range("A" & rowIndex).value
        groupName = mappingDefSheet.range("B" & rowIndex).value
        ...
        Call mappingNumberManager.addMocAttrNetype(sheetName, groupName, mocName, attrName, neType)
    Next rowIndex
End Sub
...
On Error GoTo ErrorHandler
    ...
    Call getControlAttrInfo(controldef, mocName, attrName, neType, sheetName, groupName, controlInfo)
...
Function Check_Value_Validation(ByVal CurSheet As Worksheet, ByVal cellRange As range, ByRef isConAttr As Boolean, ByRef contRel As controlRelation, ByRef currentNeType As String) As Long
    ...
    For i = 0 To ControlRelationNum - 1
        If ControlRelMap(i).mocName = mocName And ControlRelMap(i).contAttr = attrName _
            And ControlRelMap(i).neType = neType And ControlRelMap(i).sheetName = sheetName And ControlRelMap(i).groupName = groupName Then
            If mappingNumberManager.hasOneMocAttrNetypeRecord(sheetName, groupName, ControlRelMap(i).mocName, attrName, neType) Then
                isConAttr = True
...
Private Function controlledAttrValidationCheck(ByRef ws As Worksheet, ByRef controldef As CControlDef, ByRef cellRange As range) As Boolean
    ...
    Dim mocName As String, attrName As String, neType As String, controlInfo As String, sheetName As String, groupName As String
    Call getControlAttrInfo(controldef, mocName, attrName, neType, sheetName, groupName, controlInfo)
    
     '如果该参数不是被控参数，则退出
    If Not controlRelationManager.containsControlledAttributeRelation(mocName, attrName, neType, sheetName, groupName) Then Exit Function
    
    '找到该参数的所有主控参数类
    Dim controlRelation As CControlRelation
    Set controlRelation = controlRelationManager.getControlRelation(mocName, attrName, neType, sheetName, groupName)
    ...
    mainControlAttrReturnedValue = makeControlAttrValueCol(ws, mocName, attrName, neType, sheetName, groupName, controlRelation, controlAttrValueManager, cellRange)
...
Private Function makeControlAttrValueCol(ByRef ws As Worksheet, ByRef mocName As String, ByRef attributeName As String, ByRef neType As String, ByRef sheetName As String, _
    groupName As String, ByRef controlRelation As CControlRelation, ByRef controlAttrValueManager As CControlAttrValueManager, ByRef cellRange As range) As Long
    ...
    For Each eachControlAttr In controlAttrCol
        ...
        If Not controlRelationManager.containsAttributeRelation(mocName, mainControlAttr, neType, sheetName, groupName) Then
            makeControlAttrValueCol = 1 '1表示在控制范围内
            Exit Function
        End If
        
        If Not mappingNumberManager.hasOneMocAttrNetypeRecord(sheetName, groupName, mocName, mainControlAttr, neType) Then
            makeControlAttrValueCol = 1 '在表格中找到同名主控参数的多条记录，如以太网端口的两组端口属性，此时没办法进行控制，则不控制，直接退出
            Exit Function
        End If
        
        Set mainControlRelation = controlRelationManager.getControlRelation(mocName, mainControlAttr, neType, sheetName, groupName)
...
Public Sub currentParameterBranchCheck(ByRef ws As Worksheet, ByRef cellRange As range)
    ...
    Call getControlAttrInfo(controldef, mocName, attrName, neType, sheetName, groupName, controlInfo)
     ...
     '如果该参数不是被控参数，则退出
    If Not controlRelationManager.containsControlledAttributeRelation(mocName, attrName, neType, sheetName, groupName) Then Exit Sub
...
Private Sub makeControlAttrValueManager(sheet As Worksheet, dstRowNumber As Long, mainControlMocName As String, mainControlAttrName As String, _
    neType As String, virtualSheetName As String, oneMainControlAttrNotExist As Boolean, controlAttrValueManager As CControlAttrValueManager, groupName As String)
    ...
    dstColumnNumber = get_MocAndAttrcolNum(mainControlMocName, mainControlAttrName, virtualSheetName, groupName) '得到主控参数的列号
    ...
    '如果在当前页签没有找到该主控参数的控制信息，说明主控参数缺少，则该被控参数不控制
    If Not controlRelationManager.containsAttributeRelation(mainControlMocName, mainControlAttrName, neType, virtualSheetName, groupName) Then
        oneMainControlAttrNotExist = True
        Exit Sub
    End If
    ...
    Set mainControlRelation = controlRelationManager.getControlRelation(mainControlMocName, mainControlAttrName, neType, virtualSheetName, groupName)
...
Function checkAllBranchMatch(rootNode As Variant, sheet As Worksheet, cellRange As range, contRel As controlRelation, contedType As String, _
    contedOutOfControl As Boolean, contedColNum As Long, ByRef currentNeType As String, ByRef controlInfo As String) As Boolean
    On Error Resume Next
    ...
    For Each eachMainControlAttr In mainControlAttrCol
        Call makeControlAttrValueManager(sheet, cellRowNumber, contRel.mocName, CStr(eachMainControlAttr), _
            currentNeType, contRel.sheetName, oneMainControlAttrNotExist, controlAttrValueManager, contRel.groupName)
...
Private Sub getControlAttrInfo(ByRef controldef As CControlDef, ByRef mocName As String, ByRef attrName As String, _
    ByRef neType As String, ByRef sheetName As String, groupName As String, ByRef controlInfo As String)
    mocName = controldef.mocName
    attrName = controldef.attributeName
    neType = controldef.neType
    sheetName = controldef.sheetName
    groupName = controldef.groupName
...
Public Function getRealControlInfo(conInfor As String) As String
    ...
    With controlInforSht
        maxCol = .range("IV" & targetRow).End(xlToLeft).column
        Dim cell As range
