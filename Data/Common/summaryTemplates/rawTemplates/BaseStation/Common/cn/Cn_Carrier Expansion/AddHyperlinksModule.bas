...
Public Sub addHyperlinksForSheets(ByRef sheet As Worksheet)
    ...
    ElseIf isCellSheet(sheet.name) Or isSectorEqmGroupSht(sheet.name) Then
...
Private Sub addCellSheetHyperlinks_SheetActive(ByRef ws As Worksheet)
    ...
    If Not (isCellSheet(ws.name) Or isSectorEqmGroupSht(ws.name)) Or containsASheet(ThisWorkbook, DecouplingSheetName) Then
...
Private Sub makeBsBoardStyleMap(ByRef bsBoardStyleMap As CMap, ByRef cellsheet As Worksheet)
    ...
    bsNameColumnName = cellsheet.range("A2").value '得到基站名称或rat名称列，如“*BTS Name”
