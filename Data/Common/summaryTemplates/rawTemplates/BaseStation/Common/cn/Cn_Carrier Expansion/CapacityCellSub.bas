...
Sub AddSectorEqm(ByRef selectedMocCol As Collection, ByRef CellSheetName As String)
    ...
    '只需调用refreshBtsRxuBoardMap，当前防止有改漏Thisworkbook的处理，所以这里还是会插入list
...
Public Sub insertTrxColumn()
        ...
        Dim cellRang As range
        Set cellRang = ThisWorkbook.ActiveSheet.range("C2:C" + CStr(2 + ROW_COUNT + 3))
        If alltrxlistStr <> "" Then
...
Private Function findBoardNosStrByBtsName_i(ByRef brdStyleSheet As Worksheet, ByRef grpCollection As Collection, ByRef btsName As String) As String
    Dim startRow As Long
    Dim endRow As Long
    Dim index As Long
    Dim boardNoIndex As Long
...
Private Function calculateRow(ByRef chkPassed As Boolean) As Long
    ...
    For index = 3 To cellsheet.range("a1048576").End(xlUp).row
        ...
        If existInCollection(temBtsName, selectBtsNameCol) And cellsheet.Cells(index, constCellTempCol).value <> "" And cellsheet.Cells(index, 2).value <> "RMV" Then
            ...
            tem_key = cellsheet.Cells(index, btsNameColIndex).value + "*" + cellsheet.Cells(index, constCellTempCol).value
...
Private Sub getCellMultiRruCellModeMocAndAttrName(ByRef mocName As String, ByRef attrName As String)
    If CELL_TYPE = 1 Then
        mocName = "ULOCELL"
        attrName = "LOCELLTYPE"
...
Private Function insertSectorEqmGrpIdColumn() As Boolean
        ...
        Call refreshCommentText(ThisWorkbook.ActiveSheet.Cells(1, SectorEqmGrpId_Index), getResByKey("SECTOREQMCOMBINEGRPID"))
...
Private Function insertCellBeamModeColumn() As Boolean
        ...
        Dim referencedString As String
        Dim charStr As String
        referencedString = "NORMAL,ADVANCED_BEAMFORMING"
        ...
        Call refreshCommentText(ThisWorkbook.ActiveSheet.Cells(1, CellBeamMode_Index), getResByKey("CELLBEAMMODE"))
...
Private Sub writeLTEOneCell(tmpsheet As Worksheet, cellsheet As Worksheet, temKey As String, _
    ...
    If CELL_TYPE = 2 Then
        ...
        If sectorEqmGrpIdStr <> "" Then cellsheet.Cells(cellInRow, sectorEqmGrpIdIndex).value = sectorEqmGrpIdStr
        If cellbeamModeStr <> "" Then cellsheet.Cells(cellInRow, cellBeamModeIndex).value = cellbeamModeStr
    End If
