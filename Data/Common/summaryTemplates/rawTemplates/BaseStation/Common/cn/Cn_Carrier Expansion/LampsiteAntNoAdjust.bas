...
Private Function getCellShtType(activeShtName As String) As Long
    If activeShtName = "EUSECTOREQMGROUP" Or activeShtName = getResByKey("EUSECTOREQMGROUP") Then
        getCellShtType = 2
...
Public Sub insertColAndWriteData(selectedMocCol As Collection, CellSheetName As String)
    ...
    For Each temBtsName In selectBtsNameCol
        '插入单板编号
        If insertRxuBoardColumn(CStr(temBtsName)) = False Then
            Exit Sub
        End If
    Next
...
'    End If
    ...
    For index = 3 To secGrpSheet.range("a1048576").End(xlUp).row
        ...
        If existInCollection(temBtsName, selectBtsNameCol) And secGrpSheet.Cells(index, constCellTempCol).value <> "" And secGrpSheet.Cells(index, 2).value <> "RMV" Then
            ...
            tem_key = secGrpSheet.Cells(index, btsNameColIndex).value + "*" + secGrpSheet.Cells(index, constCellTempCol).value
