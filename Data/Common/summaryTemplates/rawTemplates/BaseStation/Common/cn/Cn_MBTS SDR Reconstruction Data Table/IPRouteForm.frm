...
Private Sub SetDstSheet()
    For m_rowNum = 2 To Worksheets("SHEET DEF").Range("a65536").End(xlUp).row
        If Worksheets("SHEET DEF").Cells(m_rowNum, 1).value <> "eNodeB Radio Data" And Worksheets("SHEET DEF").Cells(m_rowNum, 2).value <> "Pattern" _
            And Check_Sheet(Worksheets("MAPPING DEF").Cells(m_rowNum, 1).value) And Worksheets(Worksheets("SHEET DEF").Cells(m_rowNum, 1).value).Visible = xlSheetVisible Then
...
Private Sub DstIPGroup_Change()
    ...
    If IPRouteForm.DstIPSheet.value = "公共数据" Then
...
Private Sub DstMaskGroup_Change()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").Range("a65536").End(xlUp).row
        ...
        End If
    Next
    If IPRouteForm.DstMaskSheet.value = "公共数据" Then
...
Private Sub CommandButton1_Click()
    ...
    For I = 0 To UBound(v)
        If v(I) = "" Then
            MsgBox "请选择设置数据。", vbExclamation, "警告"
            flag = False
            Exit For
        End If
    Next
...
Private Sub CommandButton3_Click()
    ...
    For I = 3 To Worksheets("IPRouteMap").Range("a65536").End(xlUp).row
        If Worksheets("IPRouteMap").Cells(I, 1).value = "公共数据" Then
        ...
        If Worksheets("IPRouteMap").Cells(I, 6).value = "公共数据" Then
        ...
        If Worksheets("IPRouteMap").Cells(I, 1).value = "公共数据" Then
            For dstIpRowEnd = 1 To Worksheets("公共数据").Range("a65536").End(xlUp).row
                If Worksheets("公共数据").Cells(dstIpRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
...
Function Get_ColOfCommon(sheetName As String, recordRow As Long, ColName As String) As Long
    ...
    If f_flag = False Then
        ...
        For dstIpRowEnd = 1 To Worksheets(sheetName).Range("a65536").End(xlUp).row
            If Worksheets("公共数据").Cells(recordRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                Exit For
            End If
        Next
...
Function Get_IPRoute(dstIPValue As String, dstMaskValue As String) As String
    ...
    ElseIf regIp.test(dstIPValue) = False Then
        MsgBox dstIPValue & " 是非法的IP地址", vbExclamation, "警告"
    ElseIf regIp.test(dstMaskValue) = False Then
        MsgBox dstMaskValue & " 是非法的IP地址", vbExclamation, "警告"
    End If
