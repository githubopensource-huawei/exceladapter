...
Sub setRaletionBoard(sheetName As String)
    Dim maxRow As Long
    Dim maxColomn As Long
    Dim targetColNum As Long
    Dim targetColltter As String
    Dim ws As Worksheet
    Set ws = ThisWorkbook.Worksheets(sheetName)
    maxColomn = ws.Range("XFD2").End(xlToLeft).column
    'maxRow = ws.Range("a1048576").End(xlUp).row
    targetColNum = getTargetNeClounmNum(ws)
    targetColltter = getColStr(targetColNum)
    maxRow = getMaxRowNumberWithBorder(ws, targetColltter) '??μ?μ±?°óD±??òμ?×?′óDD
    If maxRow = 0 Then Exit Sub
    Dim myRange As Range
    Set myRange = ws.Range("A1:" + getColStr(maxColomn) + CStr(maxRow))
    'Set myRange = ws.UsedRange
