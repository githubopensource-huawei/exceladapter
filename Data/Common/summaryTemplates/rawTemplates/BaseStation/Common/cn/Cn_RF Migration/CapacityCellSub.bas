...
Private Sub judgeGNormalCell()
    ...
    For Each temBtsName In selectBtsNameCol
        For rowNum = 3 To maxRow
            If cellsheet.Cells(rowNum, btsNameColIndex).value = CStr(temBtsName) Then
...
Private Function insertCellIdColumn() As Boolean
        ...
        For index = 3 To cellsheet.Range("b1048576").End(xlUp).row
            ...
            If cellsheet.Cells(index, btsNameColIndex).value = temBtsName Then
...
Public Sub insertTrxColumn()
        ...
        For index = 2 To cellsheet.Range("b1048576").End(xlUp).row
            If cellsheet.Cells(index, 2).value = temBtsName Then
        ...
        Call refreshCommentText(ThisWorkbook.ActiveSheet.Cells(1, Trx_Index), getResByKey("TRXIDINFO"))
...
Public Sub insertRxuBoardList(ByRef ws As Worksheet, ByRef cellRange As Range)
    ...
    columnNumber = cellRange.column
    If columnNumber <> Board_Index Then Exit Sub
...
Private Function findBoardStyleSheet(btsIndex As Long) As String
    ...
    columnName = ThisWorkbook.Worksheets(CELL_SHEET_NAME).Cells(2, 2).value
...
Private Function findBoardStyleSheetByBtsName(ByRef btsName As String) As String
    ...
    columnName = ThisWorkbook.Worksheets(CELL_SHEET_NAME).Cells(2, btsNameColIndex).value
    siteIndex = findColNumByGrpNameAndColName(mainSheet, groupName, columnName)
    'brd index
    Call findBrdStyleGrpNameAndColName(mainSheetName, groupName, columnName)
    brdStyleIndex = findColNumByGrpNameAndColName(mainSheet, groupName, columnName)
...
Private Function calculateRow(ByRef chkPassed As Boolean) As Long
    ...
    For index = 3 To cellsheet.Range("b1048576").End(xlUp).row
        temBtsName = cellsheet.Cells(index, btsNameColIndex).value
        If existInCollection(temBtsName, selectBtsNameCol) And cellsheet.Cells(index, constCellTempCol).value <> "" Then
...
Public Function cellSheetType(sheetName As String) As Long
    ...
    Or sheetName = getResByKey("EUCELLSECTOREQM") Or sheetName = "EUCELLSECTOREQM" Or sheetName = getResByKey("EUPRBSECTOREQM") Or sheetName = "EUPRBSECTOREQM" Then
        cellSheetType = 2
    Else
        cellSheetType = -1
    End If
...
Private Sub writeCellData(ByRef cellInfoMap As CMapValueObject)
    ...
    For rowNum = 3 To maxRow
            baseStationName = cellsheet.Cells(rowNum, 2).value
