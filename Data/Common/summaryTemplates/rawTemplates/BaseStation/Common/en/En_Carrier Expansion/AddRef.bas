...
Sub addGroupAndColoum(sheetName As String, groupName As String, coloumName As String, brdStyleRefRowNum As Long, brdStyleRefColNum As Long)
        ...
        '找到Group的起始和终止列：coloumStart，coloumEnd
        ...
        '如果找到了groupName，继续找coloumName所在的列
        ...
        If coloumIndex > 0 Then   '找到了groupName和coloumName
            Exit Sub
        ElseIf coloumIndex <= 0 And groupIndex > 0 Then  '只找到了groupName，没找到coloumName
            ...
            ws.range(columStartChar + "1:" + columEndChar + "1").Merge  '合并groupName
            Application.DisplayAlerts = True
            Call addGroupNameAndColoumName(sheetName, groupName, coloumName)   '在MappingDef中新增一行记录
            ...
            End If
            setBoard (sheetName)
        ElseIf coloumIndex <= 0 And groupIndex <= 0 Then   'groupName和coloumName都没找到
            columEndChar = getColStr(coloumEnd + 1)
            ws.Columns(columEndChar + ":" + columEndChar).Insert Shift:=xlToLeft
...
Sub clearValidationAndResetBackgroundStyle(ByRef ws As Worksheet, ByRef columEndChar As String)
    ...
    Set newColumnRange = ws.range(ws.range(columEndChar & "3"), ws.range(columEndChar & maxRow))
...
Function getRefEnumValue(brdStyleRefRowNum As Long, brdStyleRefColNum As Long)
    ...
    If getRangeGroupAndColumnName(ws, brdStyleRefRowNum, brdStyleRefColNum, groupName, columnName) = True Then
...
Sub addControlDefInfo(sheetName As String, groupName As String, coloumName As String, refEnumStr As String)
    ...
    controlDef.Cells(row + 1, 10).value = "Base Station"
