...
Function isCellSheet(sheetName As String) As Boolean
    If sheetName = "GSM Cell" Or sheetName = "UMTS Cell" Or sheetName = "LTE Cell" _
        Or sheetName = getResByKey("A174") Or sheetName = getResByKey("A175") Or sheetName = getResByKey("A176") Then
        isCellSheet = True
        Exit Function
    End If
    isCellSheet = False
End Function
Public Function isBandDefinitionSht(sheetName As String) As Boolean
    If sheetName = getResByKey("BAND_DEFINITION") Then
        isBandDefinitionSht = True
        Exit Function
    End If
    isBandDefinitionSht = False
End Function

Public Function isNeighborPolicySht(sheetName As String) As Boolean
    If sheetName = getResByKey("NEIGHBOR_POLICY") Then
        isNeighborPolicySht = True
        Exit Function
    End If
    isNeighborPolicySht = False
End Function
Function isTrasnPortSheet(sheetName As String) As Boolean
    If sheetName = "GSM Cell" Or sheetName = "UMTS Cell" Or sheetName = "LTE Cell" _
        Or sheetName = getResByKey("A177") Or sheetName = getResByKey("A178") Or sheetName = getResByKey("A179") _
        Or sheetName = "GTRXGROUP" Or sheetName = getResByKey("A180") Then
...
Function Get_LteCellTemplate_Related(DlBandwidth As String, TxRxMode As String, FDDTDD As String, SA As String, NBIoTFlag As String, sheet As Worksheet, cellRange As Range, ByRef CellType As String) As String
    ...
    For m_rowNum = 2 To MappingCellTemplate.Range("C1048576").End(xlUp).row
        ...
        And neType = MappingCellTemplate.Cells(m_rowNum, 3) And (UCase(NBIoTFlag) = UCase("FALSE") Or NBIoTFlag = "") _
        Or ((UCase(NBIoTFlag) = UCase("TRUE") And UCase(MappingCellTemplate.Cells(m_rowNum, 6).value) = UCase("NB-IoT"))) Then
...
Function Get_Template_Related(DlBandwidth As String, TxRxMode As String, FDDTDD As String, SA As String, NBIoTFlag As String, sheet As Worksheet, cellRange As Range) As String
    ...
    For m_rowNum = 2 To MappingCellTemplate.Range("C1048576").End(xlUp).row
        ...
        And (SA = MappingCellTemplate.Cells(m_rowNum, 4).value Or SA = "") And (UCase(NBIoTFlag) = UCase("FALSE") Or NBIoTFlag = "") _
        Or ((UCase(NBIoTFlag) = UCase("TRUE") And UCase(MappingCellTemplate.Cells(m_rowNum, 3).value) = UCase("NB-IoT"))) Then
            cellTemplate = MappingCellTemplate.Cells(m_rowNum, 5).value
            If Contains(cellTemplateCol, cellTemplate) Or cellTemplate = "" Then GoTo NextLoop
            cellTemplateCol.Add Item:=cellTemplate, key:=cellTemplate
            If m_Str = "" Then
                 m_Str = cellTemplate
