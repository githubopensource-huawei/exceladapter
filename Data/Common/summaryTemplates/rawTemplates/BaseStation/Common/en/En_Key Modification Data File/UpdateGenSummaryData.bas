...
Function ForInsertNewSiteNameRow(groupName As String)
    ...
    With sheet
        ...
        Cells(2, innerPositionMgr.mappingDef_shtNameColNo) = getResByKey("BaseTransPort")
        Cells(2, innerPositionMgr.mappingDef_grpNameColNo) = groupName
        Cells(2, innerPositionMgr.mappingDef_colNameColNo) = getResByKey("SheetNameForSite")
        Cells(2, innerPositionMgr.mappingDef_mocNameColNo) = "NODE"
        Cells(2, innerPositionMgr.mappingDef_attrNameColNo) = "SHEETNAMEFORSITE"
        Cells(2, innerPositionMgr.mappingDef_neTypeColNo) = Cells(3, innerPositionMgr.mappingDef_neTypeColNo)
        Cells(2, 13) = Cells(3, 13)
        Cells(2, innerPositionMgr.mappingDef_readOnlyColNo) = True
    End With
End Function
        ...
        If IsExistsSheet(ReferenceSiteName) Then
            ...
            If CreateNewSheet(CurSiteName) = False Then
               If GetCell(Summary_Shet, 2, siteNameCol + 1) = getResByKey("SheetNameForSite") Then
                   Summary_Shet.Cells(CurRow, siteNameCol + 1) = ""
               End If
              Exit Sub
            End If
...
Private Function existsIUBSht(Summary_Shet As Worksheet, ByVal rowIdx As Integer, ByVal siteNameCol As Integer) As Boolean
On Error GoTo ErrorHandler
    existsIUBSht = False
    Dim siteName As String, shtName As String
    siteName = Summary_Shet.Cells(rowIdx, siteNameCol)
    shtName = siteName
    If Summary_Shet.Cells(TitleRow, siteNameCol + 1) = getResByKey("SheetNameForSite") Then
        If Summary_Shet.Cells(rowIdx, siteNameCol + 1) <> "" Then shtName = Trim(Summary_Shet.Cells(rowIdx, siteNameCol + 1).value)
    End If
...
Function IsReferenceDef(Summary_Shet As Worksheet, ReferenceColumn As Long) As Boolean
    ...
    If Trim(Summary_Shet.Cells(iRow, siteNameCol).value) <> "" Then
        Do While (Trim(Summary_Shet.Cells(iRow, siteNameCol).value) <> "")
            If existsIUBSht(Summary_Shet, iRow, siteNameCol) = False And Summary_Shet.Cells(iRow, ReferenceColumn) = "" Then
