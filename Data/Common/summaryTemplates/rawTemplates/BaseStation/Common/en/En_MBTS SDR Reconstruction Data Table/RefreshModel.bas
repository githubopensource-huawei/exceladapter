...
Sub refreshSummary()
    ...
    For index = 2 To Worksheets(ShetDef_ShetName).range("a65536").End(xlUp).row
         tmpShetName = Worksheets(ShetDef_ShetName).Cells(index, 1)
         If UCase(Worksheets(ShetDef_ShetName).Cells(index, 2)) = "LIST" And Sheets(tmpShetName).Visible < 0 And Not isTrasnPortSheet(tmpShetName) Then
             'Flag初始为0
             flag = 0
...
Function is_DelColumnName(columnName As String) As Boolean
    ...
    If (columnName = "*NodeB名称") Then
        is_DelColumnName = True
    End If
    
    If (columnName = "*BTS Name") Then
        is_DelColumnName = True
    End If
    
    If (columnName = "*BTS名称") Then
        is_DelColumnName = True
    End If
    ...
    If (columnName = "*基站名称") Then
        is_DelColumnName = True
    End If
    
    If (columnName = "*eNodeB名称") Then
         is_DelColumnName = True
    End If
    
    If (columnName = "*eNodeB Name") Then
        is_DelColumnName = True
    End If
    
    If (columnName = "*USU Name") Or (columnName = "*USU名称") Then
        is_DelColumnName = True
    End If
...
Sub ModShetType_ByName(tmpShetName As String)
    ...
    Do While (ShetDef_Shet.Cells(iRow, 1) <> "")
        If (ShetDef_Shet.Cells(iRow, 1) = tmpShetName) Then
            ShetDef_Shet.Cells(iRow, ShetTypp_Column) = "Pattern"
            Exit Sub
        End If
        iRow = iRow + 1
    Loop
