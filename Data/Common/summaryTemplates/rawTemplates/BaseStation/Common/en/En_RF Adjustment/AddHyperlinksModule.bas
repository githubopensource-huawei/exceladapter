...
Public Sub setHyperlinkSheetsRowHeight()
    ...
    For Each ws In ThisWorkbook.Worksheets
        If isBoardStyleSheet(ws) Or ws.name = GetMainSheetName() Or isCellSheet(ws.name) Or isSectorEqmGroupSht(ws.name) Then
...
Public Sub addHyperlinksForSheets(ByRef sheet As Worksheet)
    ...
    ElseIf isCellSheet(sheet.name) Or isSectorEqmGroupSht(sheet.name) Or isEuCellSectorEqmSht(sheet.name) Or isEuPrbSectorEqmSht(sheet.name) Or isEuPrbSectorEqmGrpSht(sheet.name) Then
...
Private Sub addCellSheetHyperlinks_SheetActive(ByRef ws As Worksheet)
    ...
    If Not (isCellSheet(ws.name) Or isSectorEqmGroupSht(ws.name) Or isEuCellSectorEqmSht(ws.name) Or isEuPrbSectorEqmSht(ws.name) Or isEuPrbSectorEqmGrpSht(ws.name)) Or containsASheet(ThisWorkbook, DecouplingSheetName) Then
        Exit Sub '如果不是小区页或包含解耦页签（说明表格不完整），则退出
    End If
...
Private Sub getAntenneMocNameAndAttrByCellSheetName(ByRef mocName As String, ByRef attrName As String, ByRef CellSheetName As String)
    If CellSheetName = getResByKey("GSMCellSheetName") Or CellSheetName = "GSM Cell" Then
        mocName = "GTRXGROUPSECTOREQM"
        attrName = "SECTORANTENNA"
    ElseIf CellSheetName = getResByKey("UMTSCellSheetName") Or CellSheetName = "UMTS Cell" Then
        mocName = "ULOCELLSECTOREQM"
        attrName = "SECTORANTENNA"
    ElseIf CellSheetName = getResByKey("LTECellSheetName") Or CellSheetName = "LTE Cell" Then
        mocName = "eUCellSectorEqm"
        attrName = "SECTORANTENNA"
    ElseIf CellSheetName = getResByKey("EUSECTOREQMGROUP") Or CellSheetName = "EUSECTOREQMGROUP" Then
        mocName = "EuSectorEqmGroup"
        attrName = "SECTORANTENNA"
    ElseIf CellSheetName = getResByKey("ULOCELLSECEQMGRP") Or CellSheetName = "ULOCELLSECEQMGRP" Then
    ...
    ElseIf CellSheetName = getResByKey("EUPRBSECTOREQM") Or CellSheetName = "EUPRBSECTOREQM" Then
        ...
        attrName = "SECTORANTENNA"
    ElseIf CellSheetName = getResByKey("EUPRBSECTOREQMGROUP") Or CellSheetName = "EUPRBSECTOREQMGROUP" Then
