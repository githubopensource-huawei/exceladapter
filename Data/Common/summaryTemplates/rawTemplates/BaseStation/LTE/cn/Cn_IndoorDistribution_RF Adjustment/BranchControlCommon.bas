...
End Function
'分支默认值补充
Public Sub setBranchDefualtValue(ByRef CurSheet As Worksheet)
    Dim conRowNum As Long
    If Not isBoardStyleSheet(CurSheet) Then Exit Sub
    For conRowNum = 2 To CurSheet.range("a1048576").End(xlUp).row
        If CurSheet.Cells(conRowNum, 5).value = "Insert" And CurSheet.Cells(conRowNum, 6).value = "" Then
            CurSheet.Cells(conRowNum, 6).value = 1
        Else
