...
Public Sub sortAdjustBoardTypeSheetRRUData(ByRef sheet As Worksheet)
    Dim ws As Worksheet
    Dim groupName As String
    Dim colunmNumbere As Long
    
    groupName = getGroupNamebyMocName("RRU")
    
    If isBoardStyleSheet(sheet) Then
        Call sortDatabyCustomColunmName(sheet, groupName)
    End If
        
End Sub


Public Sub sortDatabyCustomColunmName(ByRef ws As Worksheet, ByRef groupName As String)
    ...
    ws.Sort.SortFields.Add key:=ws.range("B" & groupNameStartRowNumber + 1 & ":" & "B" & groupNameEndRowNumber) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    ws.Sort.SortFields.Add key:=ws.range("C" & groupNameStartRowNumber + 1 & ":" & "C" & groupNameEndRowNumber) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    ws.Sort.SortFields.Add key:=ws.range("D" & groupNameStartRowNumber + 1 & ":" & "D" & groupNameEndRowNumber) _
        , SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    
    With ws.Sort
        .SetRange ws.range("A" & groupNameStartRowNumber + 1 & ":" & "F" & groupNameEndRowNumber)
        .Header = xlYes
        .MatchCase = False
...
Private Function getGroupNamebyMocName(ByRef mocName As String) As String
    Dim sheetDef As Worksheet
    Dim index As Long
    Dim defCollection As New Collection
    Dim mappingdefgroupName As String
    Dim mappingdefmocName As String
    ...
    For index = 2 To sheetDef.range("a1048576").End(xlUp).row
        ...
        mappingdefmocName = sheetDef.Cells(index, 4)
        If mappingdefmocName = mocName Then
            getGroupNamebyMocName = mappingdefgroupName
            Exit For
        End If
    Next
