...
Public Function IsSystemSheet(CurSheet As Worksheet) As Boolean
    ...
    If CurSheet.name = GetMainSheetName _
       ...
       Or CurSheet.name = getResByKey("CommonData") _
         Then
        IsSystemSheet = True
    End If
End Function
...
Function get_EndRow() As Long
    ...
    Do While ShetDef_Shet.Cells(iRow, innerPositionMgr.sheetDef_shtNameColNo) <> ""
        If ShetDef_Shet.Cells(iRow, innerPositionMgr.sheetDef_endRowColNo).value <> "" Then
            If Int(ShetDef_Shet.Cells(iRow, innerPositionMgr.sheetDef_endRowColNo)) > get_EndRow Then
                get_EndRow = Int(ShetDef_Shet.Cells(iRow, innerPositionMgr.sheetDef_endRowColNo).value)
            End If
        End If
        iRow = iRow + 1
    Loop
