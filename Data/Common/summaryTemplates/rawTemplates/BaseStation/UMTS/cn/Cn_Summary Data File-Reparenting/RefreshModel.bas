...
Sub refreshSummary()
    ...
    For index = 2 To Worksheets(ShetDef_ShetName).range("a65536").End(xlUp).row
         ...
         If UCase(Worksheets(ShetDef_ShetName).Cells(index, innerPositionMgr.sheetDef_shtTypeColNo)) = "LIST" And Sheets(tmpShetName).Visible < 0 And Not isTransPortSheet(tmpShetName) Then
...
Function is_DelColumnName(columnName As String) As Boolean
    ...
    ElseIf columnName = getResByKey("*NRNAME") Then
        is_DelColumnName = True
    ElseIf columnName = getResByKey("*DSAName") Then
        is_DelColumnName = True
    End If
