...
'从指定sheet页的指定行，查找指定列，返回列号,通过attrName和MocName获取所有场景列，返回字符串
Public Function getScenarioColNum(sheetName As String, recordRow As Long, attrName As String, mocName As String) As String
    On Error Resume Next
    Dim m_colNum As Long
    Dim m_rowNum As Long
    Dim ColName As String
    ...
    For m_rowNum = 2 To mappingDef.range("a1048576").End(xlUp).row
        If UCase(attrName) = UCase(mappingDef.Cells(m_rowNum, 5).value) _
           ...
           And UCase(mocName) = UCase(mappingDef.Cells(m_rowNum, 4).value) Then
            ColName = mappingDef.Cells(m_rowNum, 3).value
            colGroupName = mappingDef.Cells(m_rowNum, 2).value
            If ColName = "" Or colGroupName = "" Then
                Exit For
            End If
            Set ws = ThisWorkbook.Worksheets(sheetName)
            For m_colNum = 1 To ws.range("XFD" + CStr(recordRow)).End(xlToLeft).column
                If get_GroupName(sheetName, m_colNum) = colGroupName Then
                    If GetDesStr(ColName) = GetDesStr(ws.Cells(recordRow, m_colNum).value) Then
                        If getScenarioColNum = "" Then
                            getScenarioColNum = CStr(m_colNum)
                        Else
                            getScenarioColNum = getScenarioColNum + "," + CStr(m_colNum)
                        End If
                        Exit For
                    End If
                End If
...
'关闭批注的刷新，提高插入删除行的效率
Public Sub refreshComment(ByRef myRange As range, Optional isAutoSize As Boolean = False)
    On Error Resume Next
    Dim Cell As range
...
Public Function existInCollection(strValue As Variant, strCollection As Collection) As Boolean
...
Public Function getMocNameByGroupNameFromMapDef(groupName As String) As String
    Dim m_rowNum As Long
    Dim mappingDef As Worksheet
    
    Set mappingDef = ThisWorkbook.Worksheets("MAPPING DEF")
    getMocNameByGroupNameFromMapDef = ""
    For m_rowNum = 2 To mappingDef.range("a1048576").End(xlUp).row
        If Trim(groupName) = mappingDef.Cells(m_rowNum, 2).value Then
            getMocNameByGroupNameFromMapDef = mappingDef.Cells(m_rowNum, 4).value
            Exit For
        End If
    Next
End Function
...
'判断是否为操作符表格，直接根据小区页签是否有操作符列判断
Public Function isOperationExcel() As Boolean
    Dim ws As Worksheet
    For Each ws In ThisWorkbook.Worksheets
        If isCellSheet(ws.name) And InStr(ws.Cells(1, 1), getResByKey("Operation_Group")) <> 0 Then
...
Private Sub initBrdGroupMap()
    ...
    For rowNum = 2 To mappingDef.range("a1048576").End(xlUp).row
        If getResByKey("Board Style") = mappingDef.Cells(rowNum, 1).value Then
            ...
            If boardStyleGroupMap.hasKey(groupName) = False Then
