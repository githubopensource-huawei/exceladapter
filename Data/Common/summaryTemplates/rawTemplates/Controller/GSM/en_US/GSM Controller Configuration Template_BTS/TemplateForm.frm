    ...
    Else
        For m_rowNum = 2 To rowscount
            If MappingCellTemplate.Cells(m_rowNum, 1) = templatename _
                ...
                And (MappingCellTemplate.Cells(m_rowNum, 5) = TxRxMode.value Or TemplateForm.TxRxMode.Enabled = False) _
                And (MappingCellTemplate.Cells(m_rowNum, 4) = BandWidth.value Or TemplateForm.BandWidth.Enabled = False) Then
                MappingCellTemplate.rows(m_rowNum).Delete
                SetCellPattern
                existflg = 1
            End If
...
Private Function containsCellSheet(ByRef neType As String) As Boolean
    ...
    cellSheetNameCn = neType & getResByKey("A223")
...
Private Sub SetCellPattern()
    ...
    Else
        ...
        saText = Me.SA.text
        For m_rowNum = 2 To MappingCellTemplate.range("a1048576").End(xlUp).row
             If (bandWidthText = MappingCellTemplate.Cells(m_rowNum, 4).value Or Me.BandWidth.Enabled = False) _
                And fddTddIddText = MappingCellTemplate.Cells(m_rowNum, 6).value _
                And (TxRxMode = MappingCellTemplate.Cells(m_rowNum, 5).value Or Me.TxRxMode.Enabled = False) _
