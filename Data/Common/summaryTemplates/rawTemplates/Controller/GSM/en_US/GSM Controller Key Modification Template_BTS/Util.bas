...
Public Function WriteFile(ByVal strFileName As String, colWri As Collection, Optional bOverLay As Boolean = False, Optional bMarkSeq As Boolean = False) As Boolean
    ...
    If pos > 0 Then
        ...
        If Not NewDir(strPath) Then
            SysErr "File path" & vbCrLf & strPath & vbCrLf & getResByKey("A220")
            Exit Function
        End If
    End If
...
Public Sub ClosingProStatusbaring()
    Application.StatusBar = getResByKey("A221") '状态栏恢复正常
End Sub
...
Public Function getColNum(ByVal sheetName As String, recordRow As Long, attrName As String, mocName As String) As Long
    On Error Resume Next
...
Function is_Site(columnName As String) As Boolean
    is_Site = False
    If (columnName = "*NodeB Name" Or columnName = getResByKey("A236")) Or (columnName = "*BTS Name" Or columnName = getResByKey("A238")) Then
        is_Site = True
    End If
End Function
