...
Sub AddLink()
    ...
    For m_rowNum = 2 To mappingDef.range("a1048576").End(xlUp).row
        ...
        If Get_DesStr(textValue) = Get_DesStr("TRUE") Then
            ...
            'For n_RowNum = 3 To ws.range("a1048576").End(xlUp).row
            For n_RowNum = 3 To ws.UsedRange.rows.count
...
Function getAllMappingDefs() As Collection
    ...
    For index = 2 To sheetDef.range("a1048576").End(xlUp).row
        ...
        mpdef.sheetName = sheetDef.Cells(index, shtNameColNumInMappingDef)
        mpdef.groupName = sheetDef.Cells(index, grpNameColNumInMappingDef)
        mpdef.columnName = sheetDef.Cells(index, colNameColNumInMappingDef)
        mpdef.mocName = sheetDef.Cells(index, mocNameColNumInMappingDef)
        mpdef.attributeName = sheetDef.Cells(index, attrNameColNumInMappingDef)
        mpdef.neType = sheetDef.Cells(index, neTypeColNumInMappingDef)
...
Function getAllControlDefines() As Collection
    ...
    For index = 2 To ctrlDefSht.range("a1048576").End(xlUp).row
        Set ctlDef = New CControlDef
        ctlDef.mocName = ctrlDefSht.Cells(index, 1).value
        ctlDef.attributeName = ctrlDefSht.Cells(index, 2).value
        ctlDef.dataType = ctrlDefSht.Cells(index, 3).value
        ctlDef.bound = ctrlDefSht.Cells(index, 4).value
        ctlDef.lstValue = ctrlDefSht.Cells(index, 5).value
        ctlDef.controlInfo = ctrlDefSht.Cells(index, 6).value
        ctlDef.sheetName = ctrlDefSht.Cells(index, 7).value
        ctlDef.groupName = ctrlDefSht.Cells(index, 8).value
        ctlDef.columnName = ctrlDefSht.Cells(index, 9).value
        ctlDef.neType = ctrlDefSht.Cells(index, 10).value
