...
Public Sub AutoDeploySheetChange(ByVal sheet As Object, ByVal target As range)
    ...
    For Each colIndex In target
        If target.row > 2 And colIndex.column = connTypeCol Then
            If colIndex.value = getResByKey("commConn") Then
            ...
            ElseIf (colIndex.value = getResByKey("sslConn") Or colIndex.value = "") Then
                ...
                If Len(nameValue) <> 0 Then
                    If ((String_ValidLen < LenB(StrConv(nameValue, vbFromUnicode)))) Or (1 > LenB(StrConv(nameValue, vbFromUnicode))) Then
                        ...
                        If nResponse = vbRetry Then
                            cellRange.Select
                        End If
                        'MsgBox (getResByKey("Length") & ESN_ValidLen)   '??????
                        cellRange.value = ""
                        Exit Sub
                    End If
                End If
            ...
            For Each cellRange In columnRange
                ...
                If Len(esnValue) <> 0 Then
                    If (ESN_ValidLen <> Len(esnValue)) Then
                        nResponse = MsgBox(getResByKey("Limited Length") & ("[" & ESN_ValidLen & "]"), vbRetryCancel + vbCritical + vbApplicationModal + vbDefaultButton1, getResByKey("Warning"))
                        If nResponse = vbRetry Then
                            cellRange.Select
                        End If
                        'MsgBox (getResByKey("Length") & ESN_ValidLen)   '??????
