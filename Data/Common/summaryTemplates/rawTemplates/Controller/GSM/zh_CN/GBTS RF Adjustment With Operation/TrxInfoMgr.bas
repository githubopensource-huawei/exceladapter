...
Public Const MOC_TRXINFO = "TRXINFO"
...
Public Const ATTR_BTSNAME = "BTSNAME"
Private Const ATTR_CELLNAME = "CELLNAME"
Private Const ATTR_GLOCELLID = "GLocellId"
Public Const ATTR_ISTMPTRX = "ISTMPTRX"

Private Const MOC_GLOCELL = "GLOCELL"
Public Const MOC_GCELL = "GCELL"
Public Const MOC_GTRX = "GTRX"
...
End Function
Public Function deleteFreq(needDeletefreq As String, inputMaFreqs As String, isRFHop As Boolean) As String
    Dim freqs() As String
...
Public Function adjustBasebandFH(ByRef processFreqs As String) As String
    Dim freqs() As String
    freqs = Split(processFreqs, ",")
    Dim index As Long
    Dim onefreq As String
    ...
    For index = LBound(freqs) To UBound(freqs)
        ...
        If InStr(onefreq, "(") <> 0 Then
            ...
            Else
                deliStr = ")"
            End If
            hopIndex = Left(hopIndex, Len(hopIndex) - 1)
            hopIndex = Right(hopIndex, Len(hopIndex) - InStr(hopIndex, deliStr))
            If hopIndex <> index Then
                '用当前数组下标值替换当前跳频值
                onefreq = replaceHopIndex(onefreq, hopIndex, CStr(index))
                adjustBasebandFH = adjustBasebandFH + onefreq + ","
            Else
                adjustBasebandFH = adjustBasebandFH + onefreq + ","
            End If
        Else
        ...
        End If
    Next
    If InStr(adjustBasebandFH, ",") <> 0 Then
        adjustBasebandFH = Left(adjustBasebandFH, Len(adjustBasebandFH) - 1)
    End If
End Function

Private Function replaceHopIndex(ByRef onefreq As String, ByRef hopIndex As String, ByRef index As String) As String
    Dim temStrArr() As String
    Dim temStr As String
    temStrArr = Split(onefreq, ":")
    temStr = Replace(temStrArr(1), hopIndex, index)
    replaceHopIndex = temStrArr(0) + ":" + temStr
End Function
...
Public Function shrinkStr(inputStr As String, deliStr As String) As String
...
'    End If
    ...
    If isMuliBrdBindScenario(brdNoStr, trxPnStr) Then
        brdNoStr = MuliBrdBindDelInd(brdNoStr)
        trxPnStr = MuliBrdBindDelInd(trxPnStr)
        antPassNoStr = MuliBrdBindDelInd(antPassNoStr)
        antTennaGroupIdStr = MuliBrdBindDelInd(antTennaGroupIdStr)
...
Public Function getGTRXBTSNameCol(shtname As String) As Long
    getGTRXBTSNameCol = getColNum(shtname, 2, "BTSNAME", "GTRX")
End Function


'RRU场景判断
Public Function isRRUScenario(brdNoStr As String) As Boolean
    isRRUScenario = False
    If InStr(brdNoStr, "[") Then
        isRRUScenario = True
    End If
...
'频点多单板绑定场景判断
Public Function isMuliBrdBindScenario(brdNoStr As String, trxPnStr As String) As Boolean
    isMuliBrdBindScenario = False
    If InStr(brdNoStr, "[") And InStr(trxPnStr, "][") Then
        isMuliBrdBindScenario = True
    '当只有一个频点时，如果有"[]"也需要判断为频点对应多单板场景。
    ElseIf Left(brdNoStr, 1) = "[" And Right(brdNoStr, 1) = "]" And Left(trxPnStr, 1) = "[" And Right(trxPnStr, 1) = "]" Then
        isMuliBrdBindScenario = True
    End If
End Function

'频点多单板绑定场景，删除字符串对应位置数据
Private Function MuliBrdBindDelInd(inputStr As String) As String
    Dim strArray() As String
    ...
    Else
        ...
        For index = LBound(strArray) To UBound(strArray)
            If index <> NeedDeleteIndex Then
                MuliBrdBindDelInd = MuliBrdBindDelInd + strArray(index) + "]["
            End If
        Next
    End If
    If InStr(MuliBrdBindDelInd, "][") <> 0 Then
        '最后的一个元素会多加一 "][" 需要剔除掉
        MuliBrdBindDelInd = Left(MuliBrdBindDelInd, Len(MuliBrdBindDelInd) - 2)
    End If
    '加头和尾
