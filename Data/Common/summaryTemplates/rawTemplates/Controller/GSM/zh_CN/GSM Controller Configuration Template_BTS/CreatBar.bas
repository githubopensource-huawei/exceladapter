...
Private Sub addUniversalBar()
    Set toolbar3 = Application.CommandBars.Add(UniversalBar, msoBarTop)
    With toolbar3
        .Protection = msoBarNoResize
        .Visible = True
        With .Controls.Add(Type:=msoControlButton)
...
Private Sub deleteUniversalBar()
    ...
    For Each bar In CommandBars
        If bar.name = UniversalBar Then
            universalBarExist = True
            Exit For
        End If
    Next
    If universalBarExist Then
...
Function isIubStyleWorkBook() As Boolean
    ...
    Dim ranges As range
    If ThisWorkbook.Worksheets("SHEET DEF").Cells(1, startRowColNumInShtDef).value <> "" Then
