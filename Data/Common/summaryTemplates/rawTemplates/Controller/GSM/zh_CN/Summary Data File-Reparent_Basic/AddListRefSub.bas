...
'    getIssueDate = getResByKey("IssueDate")
'End Function
...
Function checkIubStyle(srcwb As Workbook) As Boolean
    ...
    If whetherWbHasCertainSheet(srcwb, coverName) Then
        If Not isSrcFileIubStyleWorkBook(srcwb) Then
            ...
            Exit Function
        End If
        issueDate = getResByKey("IssueDate")
        ...
        If wb.Worksheets(sh.name).Tab.colorIndex = BluePrintSheetColor Then
            ...
            For Each row In mappingDefSheet.Rows
                If row.Cells(1, shtNameColNumInMappingDef).value = sheetName And row.Cells(1, colNameColNumInMappingDef).value = sh.Cells(startPos, column).value Then
                     ...
                     mappingDef.sheetName = row.Cells(1, shtNameCol).value
                     mappingDef.groupName = row.Cells(1, grpNameCol).value
                     mappingDef.columnName = row.Cells(1, colNameCol).value
                     mappingDef.mocName = row.Cells(1, mocNameCol).value
                     mappingDef.attributeName = row.Cells(1, attrNameCol).value
                     mappingDef.neType = row.Cells(1, neTypeCol).value
