...
Function getSheetDefIndex(ByRef sheetName As String, ByRef startRow As Long, ByRef endRow As Long) As Long
    ...
    For k = 2 To sheetDef.UsedRange.rows.count
        cellValue = sheetDef.Cells(k, shtNameColNumInShtDef).value
        If cellValue = sheetName Then
            getSheetDefIndex = k
            startRow = CLng(sheetDef.Cells(k, startRowColNumInShtDef).value)
            endRow = CLng(sheetDef.Cells(k, endRowColNumInShtDef).value)
            Exit Function
        End If
    Next k
End Function
...
Private Sub changeSheetDefRowPos(count As Long, startIndex As Long)
        ...
        For index = startIndex To iCount
                If sheetDef.Cells(index, startRowColNumInShtDef).value <> "" And sheetDef.Cells(index, endRowColNumInShtDef).value <> "" Then
                    startRow = CLng(sheetDef.Cells(index, startRowColNumInShtDef).value)
                    endRow = CLng(sheetDef.Cells(index, endRowColNumInShtDef).value)
                    If index <> startIndex Then
                        sheetDef.Cells(index, startRowColNumInShtDef).value = CStr(startRow + count)
                    End If
                    sheetDef.Cells(index, endRowColNumInShtDef).value = CStr(endRow + count)
                End If
        Next
End Sub
...
Function isSrcFileIubStyleWorkBook(srcwb As Workbook) As Boolean
    On Error GoTo ErrorHandler:
    Dim ranges As Range
    If srcwb.Worksheets("SHEET DEF").Cells(1, startRowColNumInShtDef).value <> "" Then
...
Function findMappingRow(templateWorkBook As Workbook, templateSheetDef As Worksheet, address As String) As Range
    ...
    For index = 2 To count
        If templateSheetDef.Cells(index, startRowColNumInShtDef).value <> "" And templateSheetDef.Cells(index, endRowColNumInShtDef).value <> "" Then
            startPos = CLng(templateSheetDef.Cells(index, startRowColNumInShtDef).value)
            endPos = CLng(templateSheetDef.Cells(index, endRowColNumInShtDef).value)
            If startPos < iRow And iRow <= endPos Then
                sheetName = templateSheetDef.Cells(index, shtNameColNumInShtDef).value
    ...
    If sheetName <> "" And columnName <> "" Then
        ...
        iRow = -1
        For index = 2 To count
            If sheetDef.Cells(index, shtNameColNumInShtDef).value = sheetName Then
                If sheetDef.Cells(index, startRowColNumInShtDef).value <> "" And sheetDef.Cells(index, endRowColNumInShtDef).value <> "" Then
                     startPos = CLng(sheetDef.Cells(index, startRowColNumInShtDef).value)
                     endPos = CLng(sheetDef.Cells(index, endRowColNumInShtDef).value)
        ...
        If iRow > 0 Then
            ...
            For Each sh In ThisWorkbook.Worksheets
                If ThisWorkbook.Worksheets(sh.name).Tab.colorIndex = BluePrintSheetColor Then
                    For index = 2 To sh.Range("XFD" + CStr(startPos)).End(xlToLeft).column
...
Private Sub getSheetNameByMappingDef(ByRef sheetName As String, ByRef columnName As String, mappingDef As CMappingDef)
    ...
    For Each row In sh.rows
        If mappingDef.mocName = row.Cells(1, mocNameColNumInMappingDef).value And _
            mappingDef.attributeName = row.Cells(1, attrNameColNumInMappingDef).value And mappingDef.neType = row.Cells(1, neTypeColNumInMappingDef).value Then
            sheetName = row.Cells(1, shtNameColNumInMappingDef).value
            columnName = row.Cells(1, colNameColNumInMappingDef).value
            Exit Sub
        End If
    Next
    sheetName = ""
    ...
    For Each sh In wb.Worksheets
        If wb.Worksheets(sh.name).Tab.colorIndex = BluePrintSheetColor Then
            ...
            For Each row In mappingDefSheet.rows
                If row.Cells(1, shtNameColNumInMappingDef).value = sheetName And row.Cells(1, colNameColNumInMappingDef).value = sh.Cells(startPos, column).value Then
                     Set mappingDef = New CMappingDef
                     mappingDef.sheetName = row.Cells(1, shtNameColNumInMappingDef).value
                     mappingDef.groupName = row.Cells(1, grpNameColNumInMappingDef).value
                     mappingDef.columnName = row.Cells(1, colNameColNumInMappingDef).value
                     mappingDef.mocName = row.Cells(1, mocNameColNumInMappingDef).value
                     mappingDef.attributeName = row.Cells(1, attrNameColNumInMappingDef).value
                     mappingDef.neType = row.Cells(1, neTypeColNumInMappingDef).value
...
Sub deleteRefAddress(address As String, sh As Worksheet)
        ...
        cloumNum = sh.Range("XFD3").End(xlToLeft).column
...
Sub addHyperlinks()
    ...
    shtType = getSheetType(ThisWorkbook.ActiveSheet.name)
    
    If IsBluePrintSheetName(ThisWorkbook.ActiveSheet.name) Then
        addListHyperlinks
    ElseIf UCase(shtType) = "PATTERN" Then
        Load HyperlinksForm
        HyperlinksForm.Show
    End If
End Sub
...
'        For index = 3 To ws.range("a1048576").End(xlUp).row
...
'            compareBaseName = ""
'            For index = 3 To p_ws.range("a1048576").End(xlUp).row
...
Function isListSheet(sheetName As String) As Boolean
    ...
    For m_rowNum = 2 To sheetDef.Range("a1048576").End(xlUp).row
        If sheetName = sheetDef.Cells(m_rowNum, shtNameColNumInShtDef).value Then
            If UCase(sheetDef.Cells(m_rowNum, shtTypeColNumInShtDef).value) = UCase("List") Then
                isListSheet = True
            Else
                isListSheet = False
            End If
...
Function getSheetType(sheetName As String) As String
    ...
    For m_rowNum = 2 To sheetDef.Range("a1048576").End(xlUp).row
        If sheetName = sheetDef.Cells(m_rowNum, shtNameColNumInShtDef).value Then
            getSheetType = UCase(sheetDef.Cells(m_rowNum, shtTypeColNumInShtDef).value)
            Exit Function
        End If
    Next
    getSheetType = ""
