...
Sub Summary2LLD()
    ...
    For SheetNum = 1 To ThisWorkbook.Sheets.count
        If ThisWorkbook.Sheets(SheetNum).name = "COMMON" Then
            lldModelFlag = "LLD"
            Exit For
        End If
    Next
    ...
    ElseIf (lldModelFlag = "SUMMARY") And (NeedChange() <> "") Then
        '创建[COMMON]
        Sheets("Common Data").Copy Before:=Sheets("Common Data")
        ...
        For i = 1 To UBound(sheetArray)
            If sheetArray(i) = "Common Data" Then
                sheetArray(i) = "COMMON"
            End If
...
Sub setLLDInfo()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").Range("a1048576").End(xlUp).row
        ...
        If GetDesStr(sheetName) = GetDesStr("Base Station Transport Data") Or GetDesStr(sheetName) = GetDesStr("eNodeB Radio Data") Then
            ...
            End If
        ElseIf GetDesStr(sheetName) = GetDesStr("Common Data") Then
            lldFlag = GetLldFlag(sheetName, groupName, colName)
            ...
            If GetDesStr(lldFlag) <> GetDesStr("TRUE") Then
                ...
                readColEnd = Worksheets("COMMON").Range("XFD" + CStr(readRowNum)).End(xlToLeft).column
...
Function getEndRow(readRowNum As Long) As Long
        ...
        preStatus = False
        For rowCount = 1 To GetSheetUsedRows(Worksheets("COMMON"))
                curStatus = True
...
Sub copyQOSInfo()
    ...
    readRowEnd = GetSheetUsedRows(Worksheets("COMMON"))
...
Sub setSummaryInfo()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").Range("a1048576").End(xlUp).row
        ...
        If GetDesStr(sheetName) = GetDesStr("Common Data") Then
            lldFlag = GetLldFlag(sheetName, groupName, colName)
            If GetDesStr(lldFlag) = GetDesStr("TRUE") Then
                readRowNum = Get_GroupRow("COMMON", groupName) + 2
                readColNum = Get_Col("COMMON", readRowNum - 1, colName)
                writeRowNum = Get_GroupRow("Common Data", groupName) + 2
                writeColNum = Get_Col("Common Data", writeRowNum - 1, colName)
                    
                For readRowCount = 1 To GetSheetUsedRows(Sheets("COMMON"))
                    If Sheets("COMMON").Cells(readRowNum + readRowCount, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                        Exit For
                    End If
                Next
                
                For writeRowCount = 1 To GetSheetUsedRows(Sheets("Common Data"))
                    If Sheets("Common Data").Cells(writeRowNum + writeRowCount, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                        Exit For
                    End If
                Next
...
Sub setQOSInfo()
    ...
    readRowEnd = Worksheets("COMMON").Range("c1048576").End(xlUp).row
    ...
    writeRowNum = Get_GroupRow("Common Data", "DIFPRI") + 2
    Worksheets("Common Data").rows(CStr(writeRowNum) & ":" & CStr(writeRowNum)).ClearContents
    
    For m_rowNum = readRowBegin To readRowEnd
        Select Case ThisWorkbook.Sheets("COMMON").Cells(m_rowNum, 1).value
        Case "SCTP"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "SigPri")
        Case "OM(MML)"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "OMHighPri")
        Case "OM(FTP)"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "OMLowPri")
        Case "Synchronization"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "PTPPri")
        Case "QCI1"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData1Pri")
        Case "QCI2"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData2Pri")
        Case "QCI3"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData3Pri")
        Case "QCI4"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData4Pri")
        Case "QCI5"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData5Pri")
        Case "QCI6"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData6Pri")
        Case "QCI7"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData7Pri")
        Case "QCI8"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData8Pri")
        Case "QCI9"
            writeColNum = Get_Col("Common Data", writeRowNum - 1, "UserData9Pri")
        Case Else
            writeColNum = -1
        End Select
        
        If writeColNum <> -1 Then
            ThisWorkbook.Sheets("Common Data").Cells(writeRowNum, writeColNum).value = _
...
Sub setVlanClass()
    ...
    writeRowNum = 3
    For readRowNum = readRowNum To Worksheets("COMMON").Range("c1048576").End(xlUp).row
    ...
    For writeRowCount = 1 To GetSheetUsedRows(Worksheets("VLANClassPattern"))
        If Worksheets("VLANClassPattern").Cells(writeRowNum + writeRowCount, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
            Exit For
        End If
    Next
    
    For writeRowNum = writeRowNum To GetSheetUsedRows(Worksheets("VLANClassPattern"))
...
Function Get_GroupRow(sheetName As String, groupName As String) As Long
    ...
    f_flag = False
    For m_rowNum = 1 To GetSheetUsedRows(Worksheets(sheetName))
...
Function NeedChange() As String
    ...
    Next
    If changeObj = "" Then
        MsgBox getResByKey("A126"), vbExclamation, "Warning"
    End If
