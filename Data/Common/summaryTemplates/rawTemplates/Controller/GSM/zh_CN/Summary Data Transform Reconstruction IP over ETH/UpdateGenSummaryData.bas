...
Function ForNewSiteName(siteName As String, BeginRow As Long) As String
    ...
    If Len(siteName) > 31 Then
        ...
        If GetCell(BaseStationShet, 2, siteNameCol + 1) = "Sheet Name for Site" Or GetCell(BaseStationShet, 2, siteNameCol + 1) = getResByKey("A168") Then
            For index = 3 To endRowNum
        ...
        If GetCell(BaseStationShet, 2, 2) <> "Sheet Name for Site" And GetCell(BaseStationShet, 2, 2) <> getResByKey("A169") Then
            Call ForNewSiteNameColumn
            Call ForInsertNewSiteNameRow(GetCell(BaseStationShet, 1, 1))
        End If
        BaseStationShet.Cells(BeginRow, siteNameCol) = ForNewSiteName
    End If
End Function
...
Function ForNewSiteNameColumn()
    ...
    With sheet
        ...
        If bIsEng Then
            Cells(2, siteNameCol) = "Sheet Name for Site"
        Else
            Cells(2, siteNameCol) = getResByKey("A170")
        End If
    End With
End Function
...
Function ForInsertNewSiteNameRow(groupName As String)
    ...
    With sheet
        ...
        Cells(2, shtNameColNumInMappingDef) = getResByKey("BaseTransPort")
        Cells(2, grpNameColNumInMappingDef) = groupName
        If bIsEng Then
            Cells(2, colNameColNumInMappingDef) = "Sheet Name for Site"
        Else
            Cells(2, colNameColNumInMappingDef) = getResByKey("A171")
        End If
        Cells(2, mocNameColNumInMappingDef) = "NODE"
        Cells(2, attrNameColNumInMappingDef) = "SHEETNAMEFORSITE"
        Cells(2, neTypeColNumInMappingDef) = Cells(3, neTypeColNumInMappingDef)
...
Function IsReferenceDef(Summary_Shet As Worksheet, ReferenceColumn As Long) As Boolean
    ...
    If Trim(Summary_Shet.Cells(iRow, siteNameCol).value) <> "" Then
        Do While (Trim(Summary_Shet.Cells(iRow, siteNameCol).value) <> "")
            If IsExistsSheet(Trim(Summary_Shet.Cells(iRow, siteNameCol).value)) = False And Summary_Shet.Cells(iRow, ReferenceColumn) = "" Then
...
Private Function checkSameSitesName() As Boolean
    ...
    For rowNumber = NodeBBeginRow To NodeBEndRow
        ...
        Else
            ...
            Else
                MsgBox getResByKey("A172") + siteName + getResByKey("A173"), vbCritical
            End If
    ...
    Do While (Trim(Summary_Shet.Cells(iRow, siteNameCol)) <> "")
        ...
        If Len(ReferenceSiteName) > 31 Then
            Do While (Trim(Summary_Shet.Cells(index, siteNameCol)) <> "")
                If Trim(Summary_Shet.Cells(index, ReferenceColumn)) = ReferenceSiteName And Trim(Summary_Shet.Cells(index, siteNameCol + 1)) <> "" Then
                    If GetCell(Summary_Shet, 2, siteNameCol + 1) = "Sheet Name for Site" Or GetCell(Summary_Shet, 2, siteNameCol + 1) = getResByKey("A174") Then
        ...
        NewSiteName = Trim(Summary_Shet.Cells(iRow, siteNameCol))
        If Len(NewSiteName) > 31 Then
            If GetCell(Summary_Shet, 2, siteNameCol + 1) = "Sheet Name for Site" Or GetCell(Summary_Shet, 2, siteNameCol + 1) = getResByKey("A175") Then
                If Summary_Shet.Cells(iRow, siteNameCol + 1) <> "" Then
                    NewSiteName = Trim(Summary_Shet.Cells(iRow, siteNameCol + 1))
                    If Len(NewSiteName) > 31 Then
                        If bIsEng Then
                            ErrMsg = "The sheet name for site is more than 31."
                            MsgBox ErrMsg
                            Call refreshEnd
                            Exit Sub
                        Else
...
Function GenNewName(siteName As String, BeginRow As Long) As String
    ...
    If Len(siteName) > 31 Then
        ...
        If GetCell(BaseStationShet, 2, siteNameCol + 1) <> "Sheet Name for Site" And GetCell(BaseStationShet, 2, siteNameCol + 1) <> getResByKey("A176") Then
            Call ForNewSiteNameColumn
        End If
