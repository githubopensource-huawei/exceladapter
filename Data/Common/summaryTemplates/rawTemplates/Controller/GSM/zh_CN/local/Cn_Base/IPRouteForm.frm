...
Private Sub DstIPGroup_Change()
    ...
    If IPRouteForm.DstIPSheet.value = getResByKey("A74") Then
...
Private Sub DstMaskGroup_Change()
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").range("a1048576").End(xlUp).row
        ...
        End If
    Next
    If IPRouteForm.DstMaskSheet.value = getResByKey("A75") Then
...
Private Sub CommandButton1_Click()
    ...
    For i = 0 To UBound(v)
        If v(i) = "" Then
            MsgBox getResByKey("A76"), vbExclamation, getResByKey("A77")
            flag = False
            Exit For
        End If
    Next
...
Private Sub CommandButton2_Click()
    If IPRouteForm.DstIPList.ListIndex = -1 Then
        MsgBox getResByKey("A78"), vbExclamation, getResByKey("A79")
    Else
...
Private Sub CommandButton3_Click()
    ...
    For i = 3 To Worksheets("IPRouteMap").range("a1048576").End(xlUp).row
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A80") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 6).value = getResByKey("A81") Then
        ...
        If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A82") Then
            For dstIpRowEnd = 1 To Worksheets(getResByKey("A83")).range("a1048576").End(xlUp).row
                If Worksheets(getResByKey("A84")).Cells(dstIpRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
        ...
        End If
         ...
         If Worksheets("IPRouteMap").Cells(i, 1).value = getResByKey("A82") Then
            For sourceMaskRowEnd = 1 To Worksheets(getResByKey("A83")).range("a1048576").End(xlUp).row
                If Worksheets(getResByKey("A84")).Cells(dstMaskRow + sourceMaskRowEnd, iPRouteMaskCol).Borders(xlEdgeRight).LineStyle = xlNone Then
                    Exit For
                End If
            Next
...
Function Get_ColOfCommon(sheetName As String, recordRow As Long, colName As String) As Long
    ...
    If f_flag = False Then
        ...
        For dstIpRowEnd = 1 To Worksheets(sheetName).range("a1048576").End(xlUp).row
            If Worksheets(getResByKey("A85")).Cells(recordRow + dstIpRowEnd, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                Exit For
            End If
        Next
...
Function Get_IPRoute(dstIPValue As String, dstMaskValue As String) As String
    ...
    ElseIf regIp.test(dstIPValue) = False Then
        MsgBox dstIPValue & getResByKey("A86"), vbExclamation, getResByKey("A87")
    ElseIf regIp.test(dstMaskValue) = False Then
        MsgBox dstMaskValue & getResByKey("A88"), vbExclamation, getResByKey("A89")
    End If
