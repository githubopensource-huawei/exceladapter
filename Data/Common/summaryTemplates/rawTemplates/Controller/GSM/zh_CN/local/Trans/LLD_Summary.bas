...
Sub copyQOSInfo()
    ...
    readRowEnd = Worksheets("COMMON").range("a1048576").End(xlUp).row
    ...
    For m_rowNum = 2 To Worksheets("MAPPING DEF").range("a1048576").End(xlUp).row
        ...
        If GetDesStr(sheetName) = GetDesStr("Common Data") Then
            ...
            If GetDesStr(lldFlag) = GetDesStr("TRUE") Then
                ...
                For readRowCount = 1 To Sheets("COMMON").range("a1048576").End(xlUp).row
                    If Sheets("COMMON").Cells(readRowNum + readRowCount, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
                        Exit For
                    End If
                Next
                
                For writeRowCount = 1 To Sheets("Common Data").range("a1048576").End(xlUp).row
...
Sub setVlanClass()
    ...
    For writeRowCount = 1 To Worksheets("VLANClassPattern").range("a1048576").End(xlUp).row
        If Worksheets("VLANClassPattern").Cells(writeRowNum + writeRowCount, 1).Borders(xlEdgeRight).LineStyle = xlNone Then
            Exit For
        End If
    Next
    
    For writeRowNum = writeRowNum To Worksheets("VLANClassPattern").range("b1048576").End(xlUp).row
...
Function NeedChange() As String
    ...
    Next
    If changeObj = "" Then
        MsgBox "Please setting LLD parameters using Summary Customizing tool！", vbExclamation, "Warning"
    End If
