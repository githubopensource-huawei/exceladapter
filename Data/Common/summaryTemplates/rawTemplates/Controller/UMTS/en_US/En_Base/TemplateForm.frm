...
Private Sub AddSite()
    ...
    '    If (Worksheets("MappingSiteTemplate").Cells(n_RowNum, 1) = SiteType.value And Worksheets("MappingSiteTemplate").Cells(n_RowNum, 5) = neType) _
    '        Or Worksheets("MappingSiteTemplate").Cells(n_RowNum, 1) = "" Then
...
Private Sub DeleteCell()
    ...
    Else
        For m_rowNum = 2 To rowscount
            If Worksheets("MappingCellTemplate").Cells(m_rowNum, 1) = templatename _
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 2) = CellType Or Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 2))) = 0) _
                ...
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 6) = FddTddIdd.value Or Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 6))) = 0) _
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 5) = TxRxMode.value Or Me.TxRxMode.Enabled = False Or Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 5).value)) = 0) _
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 4) = BandWidth.value Or Me.BandWidth.Enabled = False Or Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 4).value)) = 0) Then
                Worksheets("MappingCellTemplate").rows(m_rowNum).Delete
                SetCellPattern
                existflg = 1
            End If
    ...
    Else
        For m_rowNum = rowscount To 2 Step -1
            If Worksheets("MappingCellTemplate").Cells(m_rowNum, 1) = templatename _
                ...
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 5) = TxRxMode.value Or Me.TxRxMode.Enabled = False) _
                And (Worksheets("MappingCellTemplate").Cells(m_rowNum, 4) = BandWidth.value Or Me.BandWidth.Enabled = False) Then
                MsgBox templatename & getResByKey("AlreadyExists"), vbExclamation, getResByKey("Warning")
                CellPatternText.SetFocus
                existflg = 1
                Exit For
...
Private Function containsCellSheet(ByRef neType As String) As Boolean
    ...
    cellSheetNameCn = neType & getResByKey("A74")
    ...
    Else
        ...
        For m_rowNum = 2 To Worksheets("MappingCellTemplate").range("a1048576").End(xlUp).row
             If (bandWidthText = Worksheets("MappingCellTemplate").Cells(m_rowNum, 4).value Or Me.BandWidth.Enabled = False Or Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 4).value)) = 0) _
                ...
                And neType = Worksheets("MappingCellTemplate").Cells(m_rowNum, 3).value Then
                If (TemplateForm.SA.Enabled = False) Or (SA = Worksheets("MappingCellTemplate").Cells(m_rowNum, 7).value) Or (Len(Trim(Worksheets("MappingCellTemplate").Cells(m_rowNum, 7).value)) = 0) Then
                    TemplateForm.CellPattern.AddItem (Worksheets("MappingCellTemplate").Cells(m_rowNum, 1).value)
                    If flag = True Then
                        CellPattern.value = Worksheets("MappingCellTemplate").Cells(m_rowNum, 1).value
                        flag = False
...
Private Sub DeleteRadio()
    ...
    For m_rowNum = 2 To rowscount
        ...
        And (Worksheets("MappingRadioTemplate").Cells(m_rowNum, 2) = radioTp Or Len(Trim(Worksheets("MappingRadioTemplate").Cells(m_rowNum, 2))) = 0) _
...
Private Sub AddRadio()
    ...
    'lastLineofGroup = rowscount
    'For n_RowNum = 2 To rowscount
    '    If (Worksheets("MappingRadioTemplate").Cells(n_RowNum, 3) = neType And Worksheets("MappingRadioTemplate").Cells(n_RowNum, 2) = radioTp) _
    '    Or Worksheets("MappingRadioTemplate").Cells(n_RowNum, 1) = "" Then
    '        lastLineofGroup = n_RowNum
    '    End If
    ' Next
