...
'''''        If UCase(attrName) = UCase(mappingDef.Cells(m_rowNum, attrNameColNumInMappingDef).value) _
'''''            And UCase(sheetName) = UCase(mappingDef.Cells(m_rowNum, shtNameColNumInMappingDef).value) _
'''''            And UCase(mocName) = UCase(mappingDef.Cells(m_rowNum, mocNameColNumInMappingDef).value) Then
'''''            colName = mappingDef.Cells(m_rowNum, colNameColNumInMappingDef).value
'''''            colGroupName = mappingDef.Cells(m_rowNum, grpNameColNumInMappingDef).value
'''''            flag = True
'''''            Exit For
'''''        End If
'''''    Next
...
Public Function GetMainSheetName() As String
    ...
    For rowNum = 1 To sheetDef.Range("a1048576").End(xlUp).row
        If sheetDef.Cells(rowNum, shtTypeColNumInShtDef).value = "MAIN" Then
            name = sheetDef.Cells(rowNum, shtNameColNumInShtDef).value
            Exit For
        End If
    Next
...
Function GetCommonSheetName() As String
    ...
    For rowNum = 1 To sheetDef.Range("a1048576").End(xlUp).row
       If sheetDef.Cells(rowNum, shtTypeColNumInShtDef).value = "COMMON" Then
           name = sheetDef.Cells(rowNum, shtNameColNumInShtDef).value
           Exit For
       End If
    Next
...
Public Sub clearXLGray()
    ...
    For index = 2 To sheetDef.Range("a1048576").End(xlUp).row
            Set worksh = ThisWorkbook.Worksheets(sheetDef.Cells(index, shtNameColNumInShtDef).value)
            If sheetDef.Cells(index, shtTypeColNumInShtDef) = "COMMON" Then
                    For commIndex = 1 To getSheetUsedRows(worksh)
                            ...
                            Next
                    Next
            ElseIf "Pattern" = sheetDef.Cells(index, shtTypeColNumInShtDef) Then
...
Public Function isPatternSheet(sheetName As String) As Boolean
    ...
    For m_rowNum = 2 To sheetDef.Range("a1048576").End(xlUp).row
        If sheetName = sheetDef.Cells(m_rowNum, shtNameColNumInShtDef).value Then
            If sheetDef.Cells(m_rowNum, shtTypeColNumInShtDef).value = "Pattern" Then
                isPatternSheet = True
            Else
...
Function is_Site(columnName As String) As Boolean
    ...
    If (columnName = getResByKey("*NodeBName") Or columnName = getResByKey("*BTSName") Or _
        ...
        columnName = getResByKey("*USUName") Or columnName = getResByKey("USU3900NAME") Or _
        columnName = getResByKey("USU3910NAME") Or columnName = getResByKey("*NBBSName") Or columnName = getResByKey("*gNodeBName") Or _
        columnName = getResByKey("*ICSName") Or columnName = getResByKey("*OneAirName") Or columnName = getResByKey("*RFAName")) Then
        is_Site = True
    End If
End Function
...
'''''Public Function GetBluePrintSheetName() As String '???????
...
'''''End Function
'''''
'''''Public Function getBaseShtName(listShtName As String) As String
'''''    getBaseShtName = GetMainSheetName
'''''End Function

'''''Public Function iubShtsWithSameBaseSht(leftListShtName As String, rightListShtName As String) As Boolean
'''''    iubShtsWithSameBaseSht = False
'''''    If getBaseShtName(leftListShtName) = getBaseShtName(rightListShtName) Then iubShtsWithSameBaseSht = True
'''''End Function

'''''Public Function mocWithSameBaseSht(leftMocName As String, rightMocName As String) As Boolean
'''''    mocWithSameBaseSht = False
'''''
'''''    Dim baseShtNameCol As Integer
'''''    baseShtNameCol = baseShtNameColNumInShtDef
'''''
'''''    Dim leftRow As Integer
'''''    Dim rightRow As Integer
'''''    leftRow = shtRowInShtDef(leftMocName)
'''''    rightRow = shtRowInShtDef(rightMocName)
'''''
'''''    Dim shtDef As Worksheet
'''''    Set shtDef = ThisWorkbook.Worksheets("SHEET DEF")
'''''
'''''    If shtDef.Cells(leftRow, baseShtNameCol) = shtDef.Cells(rightRow, baseShtNameCol) Then mocWithSameBaseSht = True
'''''End Function
