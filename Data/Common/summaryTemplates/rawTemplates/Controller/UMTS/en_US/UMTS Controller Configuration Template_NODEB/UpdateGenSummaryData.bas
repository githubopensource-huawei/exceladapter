...
Function ForNewSiteName(siteName As String, BeginRow As Long) As String
  ...
  If Len(siteName) > 31 Then
    ...
    If GetCell(BaseStationShet, 2, 2) = "Sheet Name for Site" Or GetCell(BaseStationShet, 2, 2) = getResByKey("A253") Then
      For index = 3 To endRowNum
        OriSiteName = GetCell(BaseStationShet, index, 2)
        NameEndString = Right(OriSiteName, 5)
    ...
    If GetCell(BaseStationShet, 2, 2) <> "Sheet Name for Site" And GetCell(BaseStationShet, 2, 2) <> getResByKey("A254") Then
...
Function ForNewSiteNameColumn()
    ...
    With sheet
           ...
           If bIsEng Then
             Cells(2, 2) = "Sheet Name for Site"
           Else
             Cells(2, 2) = getResByKey("A255")
           End If
    End With
End Function
...
Private Function checkSameSitesName() As Boolean
    ...
    For rowNumber = NodeBBeginRow To NodeBEndRow
        ...
        Else
            ...
            Else
                MsgBox getResByKey("A257") + siteName + getResByKey("A258"), vbCritical
            End If
...
Public Function siteNameColNum(ByRef ws As Worksheet) As Long
    ...
    For colIdx = 1 To ws.range("XFD2").End(xlToLeft).column
        If is_Site(ws.Cells(2, colIdx)) Then
...
Sub GenSiteSheetFromSummary()
    ...
    Do While (Trim(Summary_Shet.Cells(iRow, siteNameCol)) <> "")
        ...
        If Len(ReferenceSiteName) > 31 Then
            Do While (Trim(Summary_Shet.Cells(index, siteNameCol)) <> "")
              If Trim(Summary_Shet.Cells(index, ReferenceColumn)) = ReferenceSiteName And Trim(Summary_Shet.Cells(index, siteNameCol + 1)) <> "" Then
                If GetCell(Summary_Shet, 2, siteNameCol + 1) = "Sheet Name for Site" Or GetCell(Summary_Shet, 2, siteNameCol + 1) = getResByKey("A158") Then
        ...
        If Len(NewSiteName) > 31 Then
            If GetCell(Summary_Shet, 2, siteNameCol + 1) = "Sheet Name for Site" Or GetCell(Summary_Shet, 2, siteNameCol + 1) = getResByKey("A159") Then
...
Function GenNewName(siteName As String, BeginRow As Long) As String
  ...
  If Len(siteName) > 31 Then
    ...
    If GetCell(BaseStationShet, 2, 2) <> "Sheet Name for Site" And GetCell(BaseStationShet, 2, 2) <> getResByKey("A261") Then
      Call ForNewSiteNameColumn
    End If
