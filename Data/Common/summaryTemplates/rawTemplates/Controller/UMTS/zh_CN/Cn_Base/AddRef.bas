...
Public Sub setLinkRange(ByVal sh As Worksheet, ByVal target As range)
        ...
        End If
         ...
         If target.rows.count < sh.range("a1048576").End(xlUp).row Then
            rows = target.rows.count
        Else
            rows = sh.range("a1048576").End(xlUp).row
        End If
...
Private Sub addRange(linktype As ClinkType, ctlDef As CControlDef)
            ...
            Else
                Dim index As Long
                For index = 3 To Worksheets(linktype.linkSheetName).range("a1048576").End(xlUp).row
