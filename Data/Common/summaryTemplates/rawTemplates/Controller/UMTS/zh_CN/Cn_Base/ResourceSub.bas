...
Public Sub initResource()
        ...
        For index = 2 To Worksheets("SummaryRes").range("a1048576").End(xlUp).row
            key = Worksheets("SummaryRes").Cells(index, 1).value
            value = Worksheets("SummaryRes").Cells(index, 2).value
...
Public Function isContainGsmControl() As Boolean
    ...
    cover = getResByKey("Cover")
    For index = 3 To 5
        key = Worksheets(cover).Cells(index, 2).value
        reValue = getResByKey(key)
        If reValue = "BSC" Then
