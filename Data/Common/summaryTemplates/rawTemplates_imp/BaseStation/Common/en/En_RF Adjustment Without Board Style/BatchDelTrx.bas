...
Private Sub deleteTrxGroupId(RowNum As Integer, needDelIndexCollection As Collection)
    Dim m_colNum As Long
    Dim CellShtName As String
    CellShtName = ThisWorkbook.ActiveSheet.name
    '先获取TRX Group ID所在列
...
Private Sub deleteSectorIdAndAntNo(RowNum As Integer, needDelIndexCollection As Collection)
    Dim needDeletefreq As Variant
    Call changeSecIdAndAntNo(RowNum, needDelIndexCollection)
End Sub

Sub changeSecIdAndAntNo(cellRow As Integer, needDelIndexCollection As Collection)
    ...
    columnName = findColumnFromRelationDef(cellsheet.name, "GTRXGROUPSECTOREQM", "SECTORANTENNA")
    antnIndex = findColumnByName(cellsheet, columnName, 2)
    '获取扇区ID所在列
    'Call getSectorMocNameAndAttr(mocName, attrName)
    columnName = findColumnFromRelationDef(cellsheet.name, "GTRXGROUPSECTOREQM", "SECTORID")
    ...
    Dim freqIndex As Variant

    valueStr = antnStr
    For Each freqIndex In needDelIndexCollection
'        If valueStr <> antnStr Then
'            freqIndex = freqIndex - 1
'        End If
        valueStr = deleteIndexOfArr(freqIndex, valueStr)
    Next
    '删除空字符
    valueStr = deleteNullStr(valueStr)
    cellsheet.Cells(cellRow, antnIndex).value = valueStr
    
    valueStr = sectorStr
    For Each freqIndex In needDelIndexCollection
'        If valueStr <> sectorStr Then
'            freqIndex = freqIndex - 1
'        End If
        valueStr = deleteIndexOfArr(freqIndex, valueStr)
    Next
...
Private Function deleteNullStr(ByVal valueStr As String) As String
    Dim arrIndex As Long
    Dim tempStr As String
    Dim valueStrArr() As String
    
    valueStrArr = Split(valueStr, ";")
    tempStr = ""
    For arrIndex = LBound(valueStrArr) To UBound(valueStrArr)
        If valueStrArr(arrIndex) <> "" Then
            If tempStr = "" Then
                tempStr = valueStrArr(arrIndex)
            Else
                tempStr = tempStr + ";" + valueStrArr(arrIndex)
            End If
        End If
    Next
    deleteNullStr = tempStr
End Function

Private Function deleteIndexOfArr(ByVal index As Long, ByVal valueStr As String) As String
    Dim arrIndex As Long
    Dim tempStr As String
    Dim valueStrArr() As String
...
Private Sub batchDeleteFreq(ByRef customFreqCollection As Collection, ByRef controlName As String)
    ...
    For RowNum = 3 To maxRow
        ...
        If Contains(customGCellCollection, curGCellName) Then
            ...
            '手工编辑场景同步删除TRX Group ID
            Call deleteTrxGroupId(RowNum, needDelIndexCollection)
            ...
            If iseGBTSTemp() Then
                Call deleteSectorIdAndAntNo(RowNum, needDelIndexCollection)
            End If
...
End Function
Public Function iseGBTSTemp() As Boolean
    Dim cover As String
    Dim key As String
    
    cover = getResByKey("Cover")
    key = ThisWorkbook.Worksheets(cover).Cells(2, 2).value
    If key = getResByKey("GBTSRfAdjustCoverFileName") Or key = "GBTS RF Adjustment Data Workbook" Then
        iseGBTSTemp = False
    Else
        iseGBTSTemp = True
    End If
