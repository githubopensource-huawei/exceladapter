...
Public Const ATTR_TRXGROUPID = "GTRXGROUPID"
...
Public Function getFreqLstStrByGCell(GCellShtName As String, btsName As String, GLocellId As String) As String
    ...
    For index = 3 To cellsheet.range("a1048576").End(xlUp).row
        If cellsheet.Cells(index, btsNameColIndex).value = btsName _
            ...
            If Trim(trxNumStr) = "" Or Trim(trxNumStr) = "0" Then
                Call MsgBox(getResByKey("TrxNumCannotBeZeroOrEmpty") & ":" & GLocellId, vbInformation, getResByKey("Warning"))
                Exit Function
            End If
            ...
            Dim bchFreq As String
            tchFreqs = cellsheet.Cells(index, tchIndex).value
            bchFreq = cellsheet.Cells(index, bchIndex).value
            
            If Trim(bchFreq) = "" Then
                Call MsgBox(getResByKey("BchTchCannotBeEmtpy") & ":" & GLocellId, vbInformation, getResByKey("Warning"))
                Exit Function
            End If
            ...
            If Not (Trim(tchFreqs) = "") Then
                tchFreqs = "," + tchFreqs
            End If
            getFreqLstStrByGCell = cutTail(bchFreq + tchFreqs, curCellTrxNum)
            Exit Function
        End If
    Next
...
Private Sub deleteTrxGroupId()
    Dim m_colNum As Long
    '获取载频组ID所在列
    m_colNum = getColNum(CellShtName, 2, ATTR_TRXGROUPID, MOC_TRXINFO)
    If m_colNum > 0 Then
        changeAttr (m_colNum)
    End If
End Sub
