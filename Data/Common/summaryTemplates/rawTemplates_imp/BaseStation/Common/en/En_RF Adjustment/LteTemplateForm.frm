...
Private Sub UserForm_Activate()
    ...
    If getNBIOTFlag = True Then
        Me.Label16.Caption = getResByKey("Mode")
        Me.Label18.Caption = getResByKey("Mode")
        Me.Label23.Caption = getResByKey("Mode")
    End If
...
Private Sub SiteType_Change()
    ...
    If Me.SiteType.text = "DBS3900_LTE" Then
        ...
        If getNBIOTFlag = True Then
            Me.FDDTDDMode.AddItem ("NB-IoT")
            Me.FDDTDDMode.AddItem ("FDDNB-IoT")
        End If
    Else
        Me.FDDTDDMode.AddItem ("FDD")
    End If
...
Private Sub DeleteCell()
    ...
    '当前数据行数
    rowscount = MappingCellTemplate.range(getColStr(CellPatternCol) & "1048576").End(xlUp).row
    
    '遍历已有默认侯选值，找到相应项时删除。
    For m_rowNum = 2 To rowscount
        If MappingCellTemplate.Cells(m_rowNum, CellPatternCol) = templatename _
                And (MappingCellTemplate.Cells(m_rowNum, SACol) = SA.value Or Me.SA.Enabled = False) _
                And MappingCellTemplate.Cells(m_rowNum, FDDTDDCol) = FddTddIdd.value _
                And (MappingCellTemplate.Cells(m_rowNum, TxRxModeCol) = TxRxMode.value Or Me.TxRxMode.Enabled = False) _
                And (MappingCellTemplate.Cells(m_rowNum, BandwidthCol) = BandWidth.value Or Me.BandWidth.Enabled = False) Then
...
Private Sub AddCell()
    ...
    '当前数据行数
    rowscount = MappingCellTemplate.range(getColStr(CellPatternCol) & "E1048576").End(xlUp).row
    ...
    If existflg = 0 Then
        ...
        MappingCellTemplate.Cells(lastLineofGroup, BandwidthCol).value = BandWidth.value
        MappingCellTemplate.Cells(lastLineofGroup, TxRxModeCol).value = TxRxMode.value
        MappingCellTemplate.Cells(lastLineofGroup, FDDTDDCol).value = FddTddIdd.value
        MappingCellTemplate.Cells(lastLineofGroup, SACol).value = SA.value
        MappingCellTemplate.Cells(lastLineofGroup, CellPatternCol).value = templatename
        MappingCellTemplate.Cells(lastLineofGroup, CellTypeCol).value = CellType
...
Private Sub SetCellPattern()
    ...
    Call getCellTemplateCol(BandwidthCol, TxRxModeCol, FDDTDDCol, SACol, CellPatternCol, CellTypeCol, NETypeCol)
     ...
     '这个新增的容器用来去重复用的，控制器和基站小区模板解耦、CU和DU小区模板解耦，可能会导致小区模板下拉框内有重复模板
    ...
    For m_rowNum = 2 To MappingCellTemplate.range(getColStr(CellPatternCol) & "1048576").End(xlUp).row
         ...
         If BandWidth = MappingCellTemplate.Cells(m_rowNum, BandwidthCol).value _
         And FddTddIdd = MappingCellTemplate.Cells(m_rowNum, FDDTDDCol).value _
         And TxRxMode = MappingCellTemplate.Cells(m_rowNum, TxRxModeCol).value _
         Or (Me.BandWidth.Enabled = False And Me.TxRxMode.Enabled = False And FddTddIdd = MappingCellTemplate.Cells(m_rowNum, FDDTDDCol).value) Then
            If (Me.SA.Enabled = False) Or (SA = MappingCellTemplate.Cells(m_rowNum, SACol).value) Then
                ...
                Me.CellPattern.AddItem (cellTemplate)
                If flag = True Then
                    CellPattern.value = cellTemplate
                    flag = False
                End If
             End If
        End If
...
Private Sub SetFTMode()
    ...
    If getNBIOTFlag = True Then
        Me.FTMode.AddItem ("NB-IoT")
        Me.FTMode.AddItem ("FDDNB-IoT")
    End If
    Me.FTMode.text = "TDD"
...
Private Sub AddRadio()
    ...
    If checkInvalidCharacter(templatename, InvalidCharCommonStr) = False Then
        'MsgBox templatename & getResByKey("cellPatternIsEmpty"), vbExclamation, getResByKey("Warning")
        MsgBox getResByKey("TEMP_NAME_INVALID"), vbRetryCancel + vbCritical + vbApplicationModal + vbDefaultButton1, getResByKey("Warning")
        existflg = 1
        Exit Sub
    End If
