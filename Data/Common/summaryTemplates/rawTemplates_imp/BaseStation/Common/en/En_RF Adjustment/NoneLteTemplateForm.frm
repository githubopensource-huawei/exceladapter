...
Private Function containsCellSheet(ByRef neType As String) As Boolean
    ...
    cellSheetNameCn = neType & getResByKey("CELL")
...
Private Sub SetCellPattern()
    ...
    If Not isLteCellType Then
        For m_rowNum = 2 To MappingCellTemplate.range("a1048576").End(xlUp).row
            ...
            If CellType = MappingCellTemplate.Cells(m_rowNum, CellTypeCol).value _
            And neType = MappingCellTemplate.Cells(m_rowNum, NETypeCol).value Then
    ...
    Else
        ...
        For m_rowNum = 2 To MappingCellTemplate.range("a1048576").End(xlUp).row
            cellTemplate = MappingCellTemplate.Cells(m_rowNum, CellPatternCol).value
             ...
             If bandWidthText = MappingCellTemplate.Cells(m_rowNum, BandwidthCol).value _
                And fddTddIddText = MappingCellTemplate.Cells(m_rowNum, FDDTDDCol).value _
                And TxRxMode = MappingCellTemplate.Cells(m_rowNum, TxRxModeCol).value _
                And CellType = MappingCellTemplate.Cells(m_rowNum, CellTypeCol).value _
                And neType = MappingCellTemplate.Cells(m_rowNum, NETypeCol).value _
                Or (Me.BandWidth.Enabled = False And Me.TxRxMode.Enabled = False And fddTddIddText = MappingCellTemplate.Cells(m_rowNum, FDDTDDCol).value) Then
