...
Public tempselectedGroupMappingDefData As CBoardStyleMappingDefData
...
Public Sub addBoardStyleMoiFinishButton()
    ...
    Dim needFillinColumnEmptyFlag As Boolean
    needFillinColumnEmptyFlag = False
    
    Call finishCopyMigrationRec(needFillinColumnEmptyFlag)
    
    If needFillinColumnEmptyFlag = True Then Exit Sub
    ...
    Dim groupNameStartRowNumber As Long, groupNameEndRowNumber As Long, maxColumnNumber As Long
    groupNameStartRowNumber = moiRowsManager.startRowNumber
    groupNameEndRowNumber = moiRowsManager.endRowNumber
    

    Call getGroupAndColumnName(ws, ws.range(getStartLetter(ws, groupNameStartRowNumber) & groupNameStartRowNumber), groupName, columnName)
    Call clearSourceBoardNoRangesBoxList(groupName, groupNameStartRowNumber, groupNameEndRowNumber)
        
        'DTS2018041911519搬迁过后刷新超链接
    Call addBoardStyleHyperlinks_SheetActive(ws)
...
Public Sub clearAllBoardNoRanges(ByVal startRowNumber As Long, ByVal endRowNumber As Long, ByVal maxColumnNumber As Long)
    ...
    startColLetter = getStartLetter(currentSheet, startRowNumber)
    Call getGroupAndColumnName(currentSheet, currentSheet.range(startColLetter & startRowNumber), groupName, columnName)
    
    If isSourceNeNameColum(groupName, columnName) Then
        Set sourceBoardNoRanges = currentSheet.range(currentSheet.range(startColLetter & startRowNumber), currentSheet.range(startColLetter & endRowNumber))
        sourceBoardNoRanges.ClearContents
...
'????
Public Sub finishCopyMigrationRec(ByRef needFillinColumnEmptyFlag As Boolean)
    ...
    If isSourceNeNameColum(groupName, columnName) = False Then Exit Sub
    Dim currentneName As String
    currentneName = ""
    ...
    Set boardStyleNeMap = baseStationData.boardStyleNeMap
    Set neBoardStyleMap = baseStationData.neBoardStyleMap
    If boardStyleNeMap.hasKey(sheetName) Then currentneName = boardStyleNeMap.GetAt(sheetName)
    ...
    For rowNumber = groupNameStartRowNumber To groupNameEndRowNumber
        sourceNeName = ws.range(getStartLetter(ws, rowNumber) & rowNumber).value
        If sourceNeName <> currentneName And sourceNeName <> "" And neBoardStyleMap.hasKey(sourceNeName) Then
            ...
            If needCopyReccord = False Then
                If checkNeedFillInCellsFilled = False Then
                    needFillinColumnEmptyFlag = True
                    Exit Sub
                End If
            End If
...
Private Function isNeedCopyRecond(ByRef ws As Worksheet, ByRef rowNumber As Long, ByRef columnNamePositionLetterMap As CMap) As Boolean
    ...
    For Each mocNeedFillColumnNameVar In mocNeedFillColumnNameCol
                If Not columnNamePositionLetterMap.hasKey(CStr(mocNeedFillColumnNameVar)) Then GoTo NextLoop
...
Public Function getSourceGroupRowNumber(ByRef ws As Worksheet, ByRef groupName As String, ByRef sourceAttrBoardNo As CMap) As Long
...
Public Sub updateSourceBoradNobyNeName(ByRef ws As Worksheet, ByRef target As range)
    ...
    '先判断是否是扩展行状态下中，如果不是直接退出，优化数据初始化的时间耗费
    If target.Interior.colorIndex = 34 Or target.Interior.colorIndex = 40 Or target.Borders.LineStyle = xlLineStyleNone Or target.Interior.colorIndex <> 43 Then
        Exit Sub
    End If
...
'    Call allBoardStyleData.initBoardStyleDataMap
    ...
    'Target本身就是单个单元格，无需循环
    'For Each cellRange In Target
'    If Target.Interior.colorIndex = 34 Or Target.Interior.colorIndex = 40 Or Target.Borders.LineStyle = xlLineStyleNone Or Target.Interior.colorIndex <> 43 Then
'        Exit Sub
'    End If
    
    Call getGroupAndColumnName(ws, target, groupName, columnName)
    ...
    If isSourceNeNameColum(groupName, columnName) And boardStyleNeMap.hasKey(ws.name) Then
        ...
        Call updateSourceNeNameBoxList(ws, groupName, neName, columnName, target)
    End If
...
Private Sub updateNeSourceGroupColBoxList(ByRef ws As Worksheet, ByRef neName As String, ByRef selectRowNumber As Long, ByRef groupName As String, ByRef autocopyBoardNoName As String)
    Dim columnNamePositionLetterMap As CMap
    Dim columnNamePositionLetter As String
    Dim keyStr As String
    Dim valueStr As String
    ...
    If allBoardStyleDataMap.hasKey(neName) Then
        ...
        If boardStyleDataMap.hasKey(keyStr) Then
            ...
            Call setBoardStyleListBoxRangeValidation(ws.name, groupName, autocopyBoardNoName, valueStr, ws, ws.range(columnNamePositionLetter & selectRowNumber))
        Else
            With ws.range(columnNamePositionLetter & selectRowNumber).Validation
                .Delete
            End With
        End If
    Else
...
Private Function isSourceNeNameColum(ByRef groupName As String, ByRef columnName As String) As Boolean
    ...
    For index = 2 To sheetDef.range("a1048576").End(xlUp).row
        ...
        If attributeName = "SOURCENENAME" And groupName = mappingdefgroupName And columnName = mappingdefcolumnName Then
